-- Home Depot unknown prices must remain unknown in the protected Deals feed.
    ...
    select
      jsonb_strip_nulls(
        ...
        || jsonb_build_object(
          ...
          'buy_price', case when lower(coalesce(c.retailer, c.payload->>'retailer', '')) = 'home depot' and coalesce(c.buy_price, nullif(c.payload->>'buy_price','')::numeric) = 0 then null else coalesce(c.buy_price, nullif(c.payload->>'buy_price','')::numeric) end,
