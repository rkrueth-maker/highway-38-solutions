-- Universal Smart Import uses the existing tenant-safe staged import authority for
-- customers, properties, jobs, quotes, invoices, time, expenses, payments, and
-- other history. Keep the safe-merge behavior while recording a business-data
-- audit event instead of incorrectly labeling every import as customer-only.
...
begin
  ...
  insert into public.business_proof_log(
    ...
    v_run.business_id,v_uid,'BUSINESS_DATA_IMPORT_APPLY','ImportRun',
