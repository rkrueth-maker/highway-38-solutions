...
const UA="Mozilla/5.0 (Linux; Android 16) AppleWebKit/537.36 Chrome/151 Safari/537.36 H38ResellerScout/2.0.2";
const DEFAULT_TERMS=["electronics","appliances","furniture","tools","lawn mower","auto parts","sporting goods","collectibles"];
const BROAD_LANES=new Set(DEFAULT_TERMS.map(x=>x.toLowerCase()));
...
function broadTerm(term){return BROAD_LANES.has(String(term||"").toLowerCase().trim())}
function provenance(row,terms){const captured=String(row?.term||"").toLowerCase().trim();return !!captured&&terms.some(t=>String(t||"").toLowerCase().trim()===captured)}
...
async function verifyLoc(row,ctx){if(!Number.isFinite(ctx.lat)||!Number.isFinite(ctx.lon))return{...row,distance_miles:Number.isFinite(Number(row.distance_miles))?Number(row.distance_miles):null,location_verified:!!row.location_label};const supplied=Number(row.distance_miles);if(Number.isFinite(supplied)&&supplied>=0){if(supplied>ctx.radiusMiles)return null;return{...row,distance_miles:Number(supplied.toFixed(1)),location_verified:false,source_distance_reported:true}}let d=null;if(Number.isFinite(Number(row.lat))&&Number.isFinite(Number(row.lon)))d=hav(ctx.lat,ctx.lon,Number(row.lat),Number(row.lon));else if(row.location_label){const g=await geocode(String(row.location_label));if(g)d=hav(ctx.lat,ctx.lon,g.lat,g.lon)}if(d==null||!Number.isFinite(d))return{...row,distance_miles:null,location_verified:false};if(d>ctx.radiusMiles)return null;return{...row,distance_miles:Number(d.toFixed(1)),location_verified:true}}
function fbSeeds(rows,terms){const out=[];for(const x of rows.slice(0,160)){const raw=`${x?.title||""} ${x?.text||""}`.trim(),title=titleClean(String(x?.title||x?.text||"")),buy=Number(x?.price||money(raw)),url=String(x?.url||"");if(title.length<4||!(buy>0)||!url.includes("facebook.com/marketplace/item/"))continue;if(!provenance(x,terms)&&!terms.some(t=>matches(raw,t))&&!friendly(raw))continue;out.push({source:"Facebook Marketplace",term:String(x?.term||""),title,url,buy_price:buy,estimated_all_in:buy,image_url:String(x?.image_url||""),distance_miles:Number.isFinite(Number(x?.distance_miles))?Number(x.distance_miles):null,location_label:String(x?.location_label||""),detail_verified:true,source_search_bound:true,browser_session:x?.browser_session===true})}return out}
...
Deno.serve(async r=>{
  ...
  try{
    ...
    for(const row of detail.slice(0,24)){
      ...
      else if(v&&(row.source==="Craigslist"||row.source==="Facebook Marketplace")&&Number.isFinite(lat)&&Number.isFinite(lon)){sum[row.source].candidates++;locationCandidates.push({...v,stage:"needs_location",source_search_bound:true,reason:row.source==="Facebook Marketplace"?`Facebook listing was captured from Scout's Marketplace search, but exact listing location still needs verification before profit can be final.`:`Craigslist search was constrained to ${radiusMiles} mi, but exact listing distance could not be independently verified.`,comp_status:"not_checked",comp_url:compUrl(v.title),profit_verified:false})}
      else if(v&&row.source==="Facebook Marketplace"){sum[row.source].candidates++;locationCandidates.push({...v,stage:"needs_location",source_search_bound:true,reason:"Facebook listing was captured, but Scout needs a location before final profit verification.",comp_status:"not_checked",comp_url:compUrl(v.title),profit_verified:false})}
    ...
    return json(r,200,{status:"PASS",engine:"discover_then_verify_v060_broad_profit_v202",candidate_policy:"Anything returned by an explicit broad resale lane may remain visible as a candidate; BUY/MAYBE/PASS profitability still requires dependable sold evidence and location/cost truth.",auction_premium_policy:"actual_only",location:{lat:Number.isFinite(lat)?lat:null,lon:Number.isFinite(lon)?lon:null,postal:postal||null,radius_miles:radiusMiles},search_terms:terms,opportunities,candidates,candidate_count:candidates.length,rejected_profit_count:evaluated.rejected_count,auction_candidates,source_summary:sum,automatic:true,scanned_at:new Date().toISOString()});
  }catch(e){return json(r,200,{status:"PARTIAL",engine:"discover_then_verify_v060_broad_profit_v202",opportunities:[],candidates:[],candidate_count:0,auction_candidates:[],source_summary:{},warning:e instanceof Error?e.message:String(e)})}
});
