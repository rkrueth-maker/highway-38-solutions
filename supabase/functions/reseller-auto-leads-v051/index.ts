...
async function enrichOpenFacts(leads:any[],known:Set<string>){
  const candidates:any[]=[];for(const row of leads){if(!isDollarGeneral(row)||clean(row?.image_url)||artifactTitle(row?.canonical_title||row?.raw_title||row?.title))continue;const code=codeFor(row);if(!code)continue;candidates.push({row,code});if(candidates.length>=OPEN_FACTS_LIMIT)break}
  ...
  return{leads:out,photos,misses,checked,errors,requested:candidates.map(x=>x.code),stale_client_known_count:known.size};
}
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    console.log(JSON.stringify({event:"dg-image-enrichment-v051c",penny_checked:penny.checked.length,penny_direct:penny.direct,penny_photos:penny.photos,penny_misses:penny.misses,open_facts_requested:facts.requested.length,open_facts_checked:facts.checked.length,open_facts_photos:facts.photos,open_facts_misses:facts.misses,stale_client_known:facts.stale_client_known_count}));
    return json(req,200,{...payload,status:"PASS",leads:facts.leads,count:facts.leads.length,warnings,adapter_version:"dg-direct-photo-cache-heal-v051c",pennytree_direct_checked_ids:penny.checked,pennytree_direct_item_enriched_count:penny.direct,pennytree_direct_photo_enriched_count:penny.photos,pennytree_direct_miss_count:penny.misses,open_facts_requested_ids:facts.requested,open_facts_checked_ids:facts.checked,open_facts_requested_count:facts.requested.length,open_facts_photo_enriched_count:facts.photos,open_facts_miss_count:facts.misses,pennytree_direct_lookup_limit:DIRECT_LIMIT,open_facts_lookup_limit:OPEN_FACTS_LIMIT,stale_client_known_image_ids_ignored_for_recovery:facts.stale_client_known_count,photo_rule:"For photo-less Dollar General UPCs, verify a direct Penny Tree item page by barcode first; only then use its non-placeholder image. Open Facts remains a barcode-only fallback. Legacy v0.1.51 client miss IDs are ignored server-side so stale/transient misses can heal on Refresh; server-side bounded caches prevent repeated lookup storms.",artifact_rule:"Page navigation and relative-date titles remain in raw crawler evidence but are not image-enrichment candidates or actionable UI products.",view_contract:"Exact penny, near-penny, clearance, store verification, and resale validation remain separate evidence states."});
