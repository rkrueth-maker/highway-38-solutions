  ...
  try {
    ...
    if (action === "stores") {
      if (!Number.isFinite(Number(payload.lat)) || !Number.isFinite(Number(payload.lon))) {
        ...
        if (!geo.ok || !location)
          return json({ ok: true, product: "resale", source: "reseller-nearby-stores-v262", data: { status: "PARTIAL", stores: [], warning: "Enter a valid ZIP or use phone location before finding nearby stores." } });
