...
import {applySourceImage,enrichLeads,exactDollarGeneralImageSources,extractSourceImage} from './core.mjs';
...
async function fetchIdentityFor(row:any){
  ...
  for(const source of exactDollarGeneralImageSources(row)){
    try{
      const html=await fetchHtml(source.url);if(!html)continue;
      if(!String(best?.source_identity_title||'').trim())best=applySourceImage(best,html,source.url,source.provider);
      else if(!String(best?.image_url||'').trim()){
        const image=extractSourceImage(html,source.url);
        if(image)best={...best,image_url:image,image_source_url:source.url,image_source_provider:source.provider,image_source_scope:'exact_product',image_source_proof:'exact_upc_public_image_v070'};
      }
      if(String(best?.source_identity_title||'').trim()&&String(best?.image_url||'').trim())break;
    }catch{}
  }
  return best;
}
