...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    return json(req,200,{...payload,status:'PASS',leads,count:leads.length,function_slug:FUNCTION_SLUG,adapter_version:'exact-upc-image-v066',exact_source_count:exact,dg_image_count:dgImages,dg_image_fetch_attempted:hydrated.attempted,dg_image_fetch_found:hydrated.found,image_rule:'For Dollar General rows missing an image, Scout tries only deterministic public pages tied to the exact UPC and accepts only a real published product image. If no trustworthy image resolves, the row stays image-less; Scout never fabricates one.',penny_truth_rule:'Crawler/community sources discover and corroborate leads. Product images and exact source pages do not prove local store price or stock. Physical in-store UPC/register scan remains final local penny truth.'});
  }catch(e){return json(req,503,{error:e instanceof Error?e.message:String(e),function_slug:FUNCTION_SLUG,adapter_version:'exact-upc-image-v066'})}
});
