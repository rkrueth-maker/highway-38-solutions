...
async function persist(a:any,rows:Any[]){
  ...
  const fields='canonical_key,retailer,title,upc,sku,deal_type,buy_price,retail_price,image_url,image_storage_path,image_cached_at,image_source_url,image_mime,source_url,source_bucket,payload,penny_sort_at,penny_date_kind,first_seen_at,seen_count';
  const {data:existing,error:ee}=await a.from('reseller_hunt_cache').select(fields).in('canonical_key',keys);if(ee)throw ee;
  const old=new Map((existing||[]).map((x:Any)=>[x.canonical_key,x]));
  const hdTitles=[...new Set(candidates.filter(x=>retailerKey(x.r.retailer)==='home depot').map(x=>x.title))];
  const wgTitles=[...new Set(candidates.filter(x=>retailerKey(x.r.retailer)==='walgreens').map(x=>x.title))];
  const byTitle=new Map<string,Any>();
  for(const [retailer,titles] of [['Home Depot',hdTitles],['Walgreens',wgTitles]] as any[]){for(let i=0;i<titles.length;i+=100){const part=titles.slice(i,i+100);if(!part.length)continue;const {data}=await a.from('reseller_hunt_cache').select(fields).eq('active',true).eq('retailer',retailer).in('title',part);for(const x of data||[])byTitle.set(`${retailerKey(x.retailer)}|${titleKey(x.title)}`,x)}}
  let add=0,upd=0,skippedPenny=0;const now=new Date().toISOString(),out:any[]=[];
  for(const {r,key,title} of candidates){const rr=retailerKey(r.retailer),titleMatch=byTitle.get(`${rr}|${titleKey(title)}`),prior:any=old.get(key)||titleMatch||null;if(prior&&txt(prior.deal_type).toLowerCase()==='penny'){skippedPenny++;continue}
    const incomingImage=validImage(r.image_url||r.image||r.thumbnail_url),keepCached=!!prior?.image_storage_path;
    const image=keepCached?txt(prior.image_url):(incomingImage||txt(prior?.image_url));
    const payloadPrior=prior?.payload&&typeof prior.payload==='object'?prior.payload:{};
    const evidence=Array.isArray(payloadPrior.source_evidence)?payloadPrior.source_evidence:[];
    const ev={source_name:txt(r.source_name),source_url:txt(r.source_url),observed_at:txt(r.observed_at||now),confidence:txt(r.source_confidence),identity_confidence:txt(r.identity_confidence)};
    const ekey=`${ev.source_name}|${ev.source_url}|${ev.identity_confidence}`;const mergedEvidence=[...evidence.filter((x:any)=>`${txt(x?.source_name)}|${txt(x?.source_url)}|${txt(x?.identity_confidence)}`!==ekey),ev].slice(-12);
...
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response(null,{status:204,headers:cors});if(req.method!=='POST')return json({error:'POST_REQUIRED'},405);const a=admin();if(!await validSecret(req,a))return json({error:'UNAUTHORIZED'},401);try{const body=await req.json().catch(()=>({})),action=txt(body.action||'refresh');if(action!=='refresh')return json({error:'UNKNOWN_ACTION'},400);return json({ok:true,data:await run(a,txt(body.reason||'manual'))})}catch(e){return json({error:'H38_DEEP_CACHE_WORKER_ERROR',detail:e instanceof Error?e.message:String(e)},500)}});
