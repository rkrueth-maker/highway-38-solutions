...
async function signedInUser(request: Request): Promise<{ id: string }> {
  ...
  const res = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
    headers: {
      ...
      apikey: SERVICE_KEY,
      "x-client-info": "h38-walkthrough-transcription-auth-v2",
    },
...
async function requireMembership(
  ...
  if (!data) {
    throw new Error("The signed-in account is not an active member of this business.");
  }
...
async function requireSession(
  ...
  const payload = data.payload && typeof data.payload === "object"
    ...
    : {};
  if (clean(payload["Quote ID"] || payload.quoteId, 180) !== quoteId) {
...
async function requireEvidence(
  ...
  if (!data) {
    throw new Error(
      expectedType === "Video Walkthrough"
        ? "The saved walkthrough video has not reached private storage yet."
        : "The walkthrough voice track has not reached private storage yet.",
    );
  }
  const payload = data.payload && typeof data.payload === "object"
    ? data.payload as JsonObject
  ...
  const fileName = clean(
    payload["File Name"] || payload.fileName ||
      (expectedType === "Video Walkthrough" ? "walkthrough.webm" : "walkthrough-voice.webm"),
    220,
  );
  const mimeType = clean(
    payload["Mime Type"] || payload.mimeType ||
      (expectedType === "Video Walkthrough" ? "video/webm" : "audio/webm"),
    120,
  );
  const fileSize = Number(payload["File Size"] || payload.fileSize || 0);
  if (evidenceType.toLowerCase() !== expectedType.toLowerCase() || sourceType !== "site visit") {
    throw new Error(`The requested document is not ${expectedType}.`);
  }
  if (savedSession !== captureSessionId || savedQuote !== quoteId) {
    throw new Error(`${expectedType} does not match this Site Visit and quote.`);
  }
  if (!path || !path.startsWith(`${businessId}/`)) {
    throw new Error(`The private ${expectedType.toLowerCase()} storage path is invalid.`);
  }
  if (fileSize > MAX_BYTES) {
    throw new Error("This walkthrough is too large for automatic voice transcription. The original evidence remains saved privately.");
  }
  return {
    id: String(data.record_key),
    payload,
    bucket,
    path,
    fileName,
...
function savedResult(session: JsonObject, videoId: string) {
  if (
    clean(session["Walkthrough Transcript Status"], 80) !== "COMPLETE" ||
    clean(session["Walkthrough Transcript Attachment ID"], 180) !== videoId
  ) return null;
  return {
    transcript: clean(session["Walkthrough Transcript"], 24000),
    notes: {
    ...
    audioAttachmentId: clean(session["Walkthrough Voice Audio Attachment ID"], 180),
  };
}
...
function outputText(payload: JsonObject): string {
  ...
  for (const output of Array.isArray(payload.output) ? payload.output : []) {
    ...
    const content = Array.isArray((output as JsonObject).content)
...
async function organize(transcript: string, session: JsonObject) {
  const instructions = [
    ...
    "Every spoken dimension is UNVERIFIED_SPOKEN until checked with a measurement tool.",
  ...
  if (!res.ok) {
    throw new Error(clean(
      (payload.error as JsonObject | undefined)?.message || payload.message ||
        `OpenAI notes cleanup returned ${res.status}.`,
      4000,
    ));
  }
...
async function transcriptionAttempt(
  blob: Blob,
  source: Evidence,
  model: string,
) {
  ...
  form.append(
    ...
    new File([blob], source.fileName, {
      type: source.mimeType || blob.type ||
        (source.evidenceType === "Walkthrough Voice Audio" ? "audio/webm" : "video/webm"),
    }),
  );
...
async function transcribe(service: ServiceClient, source: Evidence) {
  ...
  if (downloaded.error || !downloaded.data) {
    throw new Error(downloaded.error?.message || "The private walkthrough evidence could not be downloaded for transcription.");
  }
  const blob = downloaded.data;
  if (blob.size > MAX_BYTES) {
    throw new Error("This walkthrough is too large for automatic voice transcription. The original evidence remains saved privately.");
  }
  const first = await transcriptionAttempt(blob, source, TRANSCRIBE_MODEL);
  if (first.ok) return { transcript: first.text, model: TRANSCRIBE_MODEL, recovered: false };
  ...
  if (mediaFailure && TRANSCRIBE_MODEL !== "whisper-1") {
    const fallback = await transcriptionAttempt(blob, source, "whisper-1");
    if (fallback.ok) return { transcript: fallback.text, model: "whisper-1", recovered: true };
    if (/corrupt|unsupported|audio file|decode|format/i.test(fallback.message)) {
      throw Object.assign(
        new Error("No usable microphone audio was found in this saved walkthrough. The video and review frames remain safe."),
...
async function save(
  ...
  const updated = {
    ...
    "Walkthrough Transcript Attachment ID": videoId,
    "Walkthrough Voice Audio Attachment ID": audioId,
    "Walkthrough Transcription Model": model,
...
async function proof(
  ...
  await service.from("business_proof_log").insert({
    ...
    details: {
      ...
      legacyVideoRecovery: recovered,
      notesModel: NOTES_MODEL,
      transcriptLength,
      spokenMeasurementCount: spokenCount,
...
Deno.serve(async (request: Request) => {
  if (request.method === "OPTIONS") {
    return reply(request, 200, { status: "PASS", preflight: true });
  }
  ...
  if (request.method !== "POST") {
    return reply(request, 405, { status: "FAIL", message: "POST is required." });
  }
  ...
  try {
    ...
    if (!ALLOWED_ORIGINS.has(origin)) {
      return reply(request, 403, {
        status: "FAIL",
        message: `Walkthrough transcription origin is not approved: ${origin || "missing origin"}.`,
      });
    }
    ...
    videoId = clean(body.attachmentId, 180);
    audioId = clean(body.audioAttachmentId, 180);
    if (!businessId || !captureSessionId || !quoteId || !videoId) {
      throw new Error("Business, capture session, saved quote, and walkthrough attachment are required.");
    }

    const user = await signedInUser(request);
    userId = user.id;
    await requireMembership(service, user.id, businessId);
    ...
    const existing = savedResult(session, videoId);
    if (existing) {
      return reply(request, 200, {
        status: "PASS",
        cached: true,
        provider: "Saved Site Visit transcript",
        transcriptionModel: clean(session["Walkthrough Transcription Model"], 120) || TRANSCRIBE_MODEL,
        notesModel: NOTES_MODEL,
        transcript: existing.transcript,
        notes: existing.notes,
        audioAttachmentId: existing.audioAttachmentId,
        safeguards: {
          ownerReviewRequired: true,
          spokenMeasurementsFieldVerified: false,
          automaticApproval: false,
    ...
    const spoken = await transcribe(service, source);
    const notes = spoken.transcript
      ? await organize(spoken.transcript, session)
    ...
    await proof(
      ...
      spoken.model,
      spoken.recovered,
      spoken.transcript.length,
      Array.isArray(notes.spokenMeasurements) ? notes.spokenMeasurements.length : 0,
    );
    return reply(request, 200, {
      status: "PASS",
      provider: "OpenAI Audio Transcriptions API",
      transcriptionModel: spoken.model,
      legacyVideoRecovery: spoken.recovered,
      notesModel: NOTES_MODEL,
    ...
    const errorCode = clean((error as { code?: unknown })?.code, 80) ||
      "SITE_WALKTHROUGH_TRANSCRIPTION_FAILED";
    await logError(service, businessId, userId, message, {
      captureSessionId,
      quoteId,
      attachmentId: videoId,
      audioAttachmentId: audioId,
    ...
    const status = /Auth session|invalid or expired/i.test(message)
      ? 401
      : /origin is not approved|role cannot|not an active member/i.test(message)
      ? 403
      : errorCode === "NO_USABLE_AUDIO"
      ? 422
      : 400;
    return reply(request, status, { status: "FAIL", errorCode, message });
  }
});
