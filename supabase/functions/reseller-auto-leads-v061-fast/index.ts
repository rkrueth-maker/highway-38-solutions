...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    return json(req,200,{...payload,status:payload?.status||"PASS",leads,count:leads.length,adapter_version:"fast-text-evidence-feed-v061",photo_mode:"deferred",photo_rule:"Normal Hunt does not call photo-enrichment providers. Existing source images may pass through, but missing pictures never delay or block discovery.",view_contract:"Crawler discovery evidence, store verification, resale evidence, and optional image evidence remain independent."});
  }catch(e){return json(req,503,{error:e instanceof Error?e.message:String(e),adapter_version:"fast-text-evidence-feed-v061",photo_mode:"deferred"})}
});
