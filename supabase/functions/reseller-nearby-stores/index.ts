...
async function overpassQuery(query:string){
  ...
  for(const endpoint of OVERPASS){
    try{
      const r=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded","user-agent":"H38-Private-Reseller-Scout/0.1.2"},body:`data=${encodeURIComponent(query)}`,signal:AbortSignal.timeout(25000)});
      if(!r.ok){failures.push(`${new URL(endpoint).host}:${r.status}`);continue;}
      return {payload:await r.json(),source:new URL(endpoint).host};
    }catch(e){failures.push(`${new URL(endpoint).host}:${e instanceof Error?e.message:String(e)}`);}
  }
  throw new Error(`Nearby store services unavailable (${failures.join(", ")}).`);
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if(prior && Date.now()-prior.at<CACHE_TTL_MS) return json(req,200,{...prior.payload,cached:true});
    const meters = Math.round(radiusMiles * 1609.344);
    const q = `[out:json][timeout:22];(nwr(around:${meters},${lat},${lon})["name"~"Home Depot|Lowe|Walmart|Target|Menards|Fleet Farm|Harbor Freight|Tractor Supply|Dollar General|Dollar Tree|Family Dollar|Northern Tool|Ace Hardware",i];nwr(around:${meters},${lat},${lon})["brand"~"Home Depot|Lowe|Walmart|Target|Menards|Fleet Farm|Harbor Freight|Tractor Supply|Dollar General|Dollar Tree|Family Dollar|Northern Tool|Ace Hardware",i];);out center tags qt;`;
    const result=await overpassQuery(q), p:any=result.payload;
    ...
    }).filter(Boolean).filter((x:any)=>x.distance_miles<=radiusMiles).sort((a:any,b:any)=>a.distance_miles-b.distance_miles).slice(0,160);
    const payload={status:"PASS",radius_miles:radiusMiles,stores,source:result.source,cached:false};
