...
for(const rel of ['commercial-app/bridge.js','commercial-app/startup-fix.js','commercial-app/service-worker.js']){const result=cp.spawnSync(process.execPath,['--check',path.join(root,rel)],{encoding:'utf8'});check(`syntax ${rel}`,result.status===0,(result.stderr||result.stdout||'').trim());}
...
check('authorized runtime acceptance exists',startupServer.includes('function cbStartupAcceptance()')&&startupServer.includes("status:'PASS'")&&startupServer.includes('cbRequireOwner_()'));
...
const output={status:failures.length?'FAIL':'PASS',checks:14,failures,fastSignedInStartup:true,ownerOnlyBusinessSwitcher:true,officeBeforeFullRefresh:true,authorizedRuntimeAcceptance:true};
