...
for (const token of ["'./site-scanner.js'","'./site-scanner.css'","'./site-scanner-mobile-guidance.js'","'./android-native-walkthrough-guard.js'","'./site-visit-top-action.js'","h38-business-office-20260809-1848","'supabase-auth.js'","'supabase-invite-activation.js'"]) must(worker, token, 'service worker');
for (const token of ["h38:auth-cache-repair","getRegistrations","reg.unregister()","key.startsWith('h38-business-office-')","location.reload()"] ) must(config, token, 'auth cache self-repair');
...
for (const token of ["versionCode 16","versionName '0.5.11'","owner {","applicationIdSuffix '.test'",'androidx.credentials:credentials:1.6.0-beta02','androidx.camera:camera-camera2:1.5.3','androidx.camera:camera-video:1.5.3','androidx.camera:camera-view:1.5.3']) must(gradle, token, 'Android v0.5.11 config');
...
for (const token of ['getRecoveredWalkthroughUrl','confirmRecoveredWalkthroughConsumed','CredentialManager','GetPasswordOption','PasswordCredential','fillWebLogin','h38:saved-login-filled','disableWebAutofill','IMPORTANT_FOR_AUTOFILL_NO_EXCLUDE_DESCENDANTS','h38:saved-login-unavailable']) must(androidBridge, token, 'Android JS bridge');
absent(androidBridge, 'activity.requestWebAutofill();', 'Android credential fallback');
for (const token of ['nativeBridge','requestAutofill','Use saved username and password','h38:saved-login-filled','h38:saved-login-unavailable','disableWebAutofillInterference','invitationPending','h38RecoveryForm','clearInvitationFlag']) must(authAutofill, token, 'Android auth and invitation activation');
