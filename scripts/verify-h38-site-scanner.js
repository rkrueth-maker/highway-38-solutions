...
for (const token of ["'./site-scanner.js'","'./site-scanner.css'","'./site-scanner-mobile-guidance.js'","'./android-native-walkthrough-guard.js'","'./site-visit-top-action.js'","h38-business-office-20260809-1605"]) must(worker, token, 'service worker');
...
for (const token of ["versionCode 12","versionName '0.5.7'",'androidx.camera:camera-camera2:1.5.3','androidx.camera:camera-video:1.5.3','androidx.camera:camera-view:1.5.3']) must(gradle, token, 'Android v0.5.7 config');
...
console.log(JSON.stringify({status:'PASS',feature:'H38 Site Scanner',databaseAuthority:'existing Supabase Business Office',browserFoundation:true,oneTapScannerStart:true,guidedFeetAndInchesMobile:true,androidCaptureSource:true,androidBusinessOfficeStartup:true,androidNativeWalkthroughVideoCapture:true,androidInAppCameraPreview:true,androidInAppMicrophone:true,androidTorchControl:true,androidPrivateWalkthroughStorage:true,externalCameraHandoff:false,androidActivityRestartRecovery:true,androidRecoveredVideoIngest:true,androidReturnToNextCaptureStep:true,androidVersion:'0.5.7',retiredFieldModeStartup:false,appleLidarSource:true,ownerReviewRequired:true,automaticApproval:false,automaticCustomerSending:false}, null, 2));
