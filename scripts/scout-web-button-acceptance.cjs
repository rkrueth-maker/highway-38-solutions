...
async function shellAcceptance(browser) {
  ...
  await page.waitForSelector('#engine-card:not(.hidden)',{timeout:30000}).catch(async()=>{throw new Error('Shell entitlement render did not expose Best Opportunities — '+(await page.locator('#authstatus').innerText().catch(()=>'')));});
  await page.waitForFunction(()=>document.querySelectorAll('#products [data-product] a.open:not(.hidden)').length===3,null,{timeout:30000});
...
async function resaleAcceptance(browser, session) {
  ...
  async function scanTab(tab, requireRows) {
    if(tab!=='deals'){
      ...
      if(await sources.count() && !(await sources.evaluate(el=>el.open))) await sources.locator('summary').click();
    }
...
async function maintenanceAcceptance(browser, session) {
  ...
  await page.waitForFunction(()=>/^(Finished\.|Maintenance failed:)/.test(document.querySelector('#msg')?.textContent||''),null,{timeout:90000});
  const autoMsg=await page.locator('#msg').innerText();
  check('Maintenance auto-check finished',/^Finished\./.test(autoMsg),autoMsg);
  check('Maintenance auto-check result',await page.locator('#report .card').count()>0,autoMsg);
  await page.click('#check');
  await waitEnabled(page,'#check',90000);
  const checkMsg=await page.locator('#msg').innerText();
  check('Maintenance Check everything',/^Finished\./.test(checkMsg),checkMsg);
