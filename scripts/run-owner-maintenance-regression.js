...
async function call(identity,body,timeout=180000,retryAuth=true){const response=await fetch(identity.endpoint,{method:'POST',headers:{...identity.headers,'Content-Type':'application/json',Origin:'https://highway38solutions.com'},body:JSON.stringify(body),signal:AbortSignal.timeout(timeout)});const raw=await response.text();let payload={};try{payload=raw?JSON.parse(raw):{};}catch{payload={status:'FAIL',message:raw.slice(0,1000)};}if(response.status===401&&identity.refreshable&&retryAuth){const refreshed=await auth(true);identity.headers=refreshed.headers;return call(identity,body,timeout,false);}if(!response.ok||payload.status!=='PASS')throw new Error(`Owner maintenance ${body.action} failed (${response.status}): ${payload.message||raw.slice(0,500)}`);return payload;}
...
(async()=>{
  ...
  report.statusAfterSeed=await call(identity,{action:'status'},30000);
  if(!sameManifest(report.status,report.statusAfterSeed)){
    report.manifestConvergedAfterSeed=true;
    report.status=report.statusAfterSeed;
  }
  report.expectedQuoteIds=Array.isArray(report.status.quoteIds)?report.status.quoteIds.slice():[];
  report.statusCoverage=assertExactCoverage(report.expectedQuoteIds,report.expectedQuoteIds,'Status manifest');save(report);
