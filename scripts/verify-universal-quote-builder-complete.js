...
const reviewClientPath='apps-script/business-office/BusinessOffice_UniversalQuoteBuilder_Control_Client.html';
const indexPath='apps-script/business-office/BusinessOffice_QuoteBuilder_Index.html';
[catalogPath,recordsPath,controlsPath,clientPath,reviewClientPath,indexPath].forEach(file=>check(`required file ${file}`,exists(file)));
if(failures.length)process.exit(1);
const catalog=read(catalogPath),records=read(recordsPath),controls=read(controlsPath),client=read(clientPath),reviewClient=read(reviewClientPath),index=read(indexPath),server=catalog+'\n'+records+'\n'+controls;
...
for(const [label,html] of [['workspace',client],['Owner review',reviewClient]]){
  const scripts=[...html.matchAll(/<script>([\s\S]*?)<\/script>/g)].map(m=>m[1]).join('\n');
  try{new Function(scripts);check(`${label} client source parses`,true);}catch(error){check(`${label} client source parses`,false,error.message);}
}
...
['boUniversalReviewProject','boUniversalCompleteConsistencyReview','boUniversalApproveRecord','boUniversalActivateKnowledgePack','boUniversalGenerateCompletionPackage','boUniversalPrepareChangeOrderRecord'].forEach(name=>check(`Owner review UI calls ${name}`,reviewClient.includes(name)));
check('Owner review interface covers projects sub-quotes drawings and bid packages',['Project|','Subquote','Drawing','Bid Package'].every(marker=>reviewClient.includes(marker)));
check('Owner review workspace is responsive',reviewClient.includes('@media(max-width:850px)'));
...
check('Owner review workspace is loaded after operational workspace',index.indexOf('BusinessOffice_UniversalQuoteBuilder_Workspace_Client')<index.indexOf('BusinessOffice_UniversalQuoteBuilder_Control_Client'));
...
check('no new app, external database, or customer send is introduced',!/(SupabaseClient|createClient\(|Firebase|sendEmail|GmailApp|MailApp)/.test(records+controls+client+reviewClient));
const result={status:failures.length?'HOLD':'PASS',generatedAt:new Date().toISOString(),architecture:'one-business-office-one-shared-quote-builder',quoteLevels:levels,pricingMethods,agents,examples,structuredRecordTypes:stores,documentTypes:docTypes.length,workspaceTabs:tabs.length,ownerReviewWorkspace:true,externalActionsPerformed:false,passed:passes.length,failed:failures.length,passes,failures};
