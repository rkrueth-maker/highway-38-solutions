...
check('Universal Quote Builder demonstrates matched public packages',universal.includes('Universal Quote Builder overview')&&universal.includes('Complete quote examples matched to their CAD drawings')&&universal.includes('?publicUqbDemo=1')&&publicRoute.includes('boRenderUniversalPublicExamples_'));
check('Universal demonstration provides all three package actions',universal.includes('View full quote')&&universal.includes('View full-size CAD sheets')&&universal.includes('Print or save the package')&&publicExamples.includes('Print / save complete package'));
check('Universal demonstration preserves public-only safety boundary',universal.includes('Public examples only:')&&universal.includes('Demonstration control:')&&publicExamples.includes('No live customers, private Highway 38 records')&&publicExamples.includes("quote['Customer Visible']==='Yes'")&&publicDemo.includes('externalActionsPerformed:false'));
check('Universal demonstration assigns seven packages and ten drawings exactly once',(publicExamples.match(/Object\.freeze\(\{key:'/g)||[]).length===7&&[...publicExamples.matchAll(/sheets:\[([^\]]+)\]/g)].flatMap(match=>(match[1].match(/'[^']+'/g)||[])).length===10);
check('Universal demonstration removes the result-board and renovation tangents',!universal.includes('What Office creates')&&!universal.includes('What Quote Builder produced')&&!universal.includes('Whole-House Renovation and Property Improvement')&&!universal.includes('$342,815'));
check('Office demo defines complete records',(publicDemo.match(/Object\.freeze\(\{n:'\d{2}',key:/g)||[]).length===14&&(publicDemo.match(/Object\.freeze\(\{n:'(?:G|A|M|P|E|C-S-L)-/g)||[]).length===10);
...
const result={status:failures.length?'HOLD':'PASS',generatedAt:new Date().toISOString(),passed:passes.length,failed:failures.length,architecture:'project-first-matched-public-quote-cad-packages',pricingProducts:3,existingProjectExamples:existingExampleCount,universalDemonstration:true,officeGeneratedDemo:true,publicMatchedPackages:7,catalogCompatibilityOnly:true,passes,failures};
