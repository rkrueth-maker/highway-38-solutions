...
const expectedPrimary=['index.html','sample-library-now.html','solutions.html','software.html','project-services.html','pricing.html','quote-builder-demo.html','implementation.html','security-reliability.html','about.html','contact.html','start-request.html','portal.html'];
check('competitive-readiness route manifest owns the thirteen current gateways',expectedPrimary.every(routePath=>primary.includes(routePath))&&primary.length===13,JSON.stringify(primary));
...
check('one canonical public shell owns balanced navigation and Project Examples',['Software','Project Services','Project Examples','Pricing','About','Owner Access','Start a Project'].every(label=>shell.includes(label))&&!shell.includes("{href:'quote-builder.html',label:'Quote Builder'}")&&!shell.includes("['Quote Builder','quote-builder.html']")&&!shell.includes("{href:'business-systems.html',label:'Business Office'}")&&!shell.includes("['Business Office','business-systems.html']")&&shell.includes("{href:'quote-builder.html#examples',label:'Project Examples'}")&&shell.includes('class="pi-menu"'));
...
const result={status:failures.length?'HOLD':'PASS',generatedAt:new Date().toISOString(),passed:passes.length,failed:failures.length,architecture:'competitive-readiness-split-buyer-paths-with-interactive-quote-proof',pricingProducts:3,existingProjectExamples:existingExampleCount,universalDemonstration:true,interactiveQuoteDemo:true,publicPlacement:'Quote Builder examples plus browser-only interactive demo',officeGeneratedDemo:true,publicMatchedPackages:packages.length,publicCadSheets:drawings.length,catalogCompatibilityOnly:true,passes,failures};
