...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('.h38-customer-tabs [data-h38-customer-tab]').count(),4,'Owner Customer 360 should expose Overview, Work, Money and Files');
    assert.deepEqual(await page.locator('.h38-customer-tabs [data-h38-customer-tab]').allTextContents(),['Overview','Work','Money','Files']);
    assert.equal(await page.locator('#h38CustomerReadyHero [data-h38-customer-action="message"]').count(),1,'Customer 360 should keep Message in customer context');
    assert.equal(await page.locator('#h38CustomerReadyHero [data-h38-customer-navigate]').count(),1,'Customer 360 should keep Navigate in customer context');
    ...
    assert((await page.locator('#h38CustomerReadyHero .h38-customer-overview-strip').textContent()).includes('$125.00'),'Customer 360 overview should show authorized billing balance');
