...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('#h38CustomerReadyToday .h38-today-primary').count(),1,'Today should have one dominant Next Action');
    assert((await page.locator('#h38CustomerReadyToday .h38-today-primary').textContent()).includes('NEXT ACTION'),'Today should explain the next action');
    assert.equal(await page.locator('#h38CustomerReadyToday .h38-today-section').count(),3,'Today should contain Up Next, Today work and Needs Attention');
    ...
    assert((await page.locator('.h38-c360').textContent()).includes('Johnson'),'Customer 360 header should identify the customer');
    assert.equal(await page.locator('.h38-c360-tabs [data-c360-tab]').count(),4,'Owner Customer 360 should expose Overview, Work, Money and Files');
    assert.deepEqual(await page.locator('.h38-c360-tabs [data-c360-tab]').allTextContents(),['Overview','Work','Money','Files']);
    assert.equal(await page.locator('.h38-c360 [data-c360-action="message"]').count(),1,'Customer 360 should keep Message in customer context');
    assert.equal(await page.locator('.h38-c360 [data-c360-action="navigate"]').count(),1,'Customer 360 should keep Navigate in customer context');
    await page.locator('.h38-c360-tabs [data-c360-tab="work"]').click();
    assert.equal(await page.locator('[data-c360-panel="work"]:not([hidden])').count(),1,'Work tab should reveal related work');
    await page.locator('.h38-c360-tabs [data-c360-tab="files"]').click();
    assert.equal((await page.locator('[data-c360-panel="files"] [data-h38-open-document-id="D-JOHN"]:not([disabled])').count())>=1,true,'Files tab must keep linked documents openable');
    await page.locator('.h38-c360-tabs [data-c360-tab="money"]').click();
    assert((await page.locator('[data-c360-panel="money"]').textContent()).includes('$125.00'),'Customer 360 Money should show authorized billing balance');
    assert.equal(await page.locator('#h38CustomerReadyCards').count(),0,'legacy customer summary cards must not remain below Customer 360');
    ...
    assert.equal(await page.locator('#h38CustomerReadyCards').count(),0,'late customer hooks must not recreate a second customer experience');
    assert.equal(await page.evaluate(()=>document.querySelector('.page-head').nextElementSibling?.classList.contains('h38-c360-master-detail')),true,'canonical Customer 360 master/detail must remain directly below the customer page heading');
    assert.equal(await page.evaluate(()=>{const master=document.querySelector('.h38-c360-master-detail'),directory=document.querySelector('.h38-c360-directory-rail'),detail=document.querySelector('.h38-c360-detail');return !!master&&directory?.parentElement===master&&detail?.parentElement===master;}),true,'Customer 360 directory and detail must stay inside the same master/detail container');
    await page.locator('.h38-c360 [data-c360-action="site"]').click();
    ...
    await page.waitForSelector('.h38-c360-workspace');
    await page.locator('.h38-c360-actions [data-c360-action="meeting"]').click();
    ...
    console.log(JSON.stringify({status:'PASS',checks:['universal New','Today next-action hierarchy','Up Next','Today work','Needs Attention','Customer 360 tabs','Customer 360 context actions','Customer files','customer-context site visit','customer-context meeting','site visit capture summary','role safety']},null,2));
