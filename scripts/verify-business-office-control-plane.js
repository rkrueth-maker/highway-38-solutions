...
check('closed-environment Social capability is Owner authorized',pack.workflow&&pack.workflow.closedEnvironment===true&&pack.modules&&pack.modules.social===true&&pack.social&&pack.social.provider==='none'&&pack.social.externalActionsEnabled===true&&pack.social.automaticPublishingEnabled===true&&pack.social.bulkPublishingEnabled===true&&pack.social.ownerApprovalRequired===true);
const controlSource=[read('rules'),read('equipmentRules'),read('core'),read('live'),read('equipmentCore'),read('portalServer')].join('\n');
check('control services contain no hidden provider network request',!/UrlFetchApp\s*\.\s*fetch|fetch\s*\(\s*['"]https?:/i.test(controlSource));
check('record-addressed social action remains available without a category policy lock',/function boControlSocialAction_\(recordId/.test(read('core'))&&pack.workflow.bulkExternalActionsEnabled===true&&pack.workflow.automaticExternalTriggersEnabled===true&&read('rules').includes("if(event==='PUBLISH')")&&read('live').includes('providerConfigured'));
...
check('equipment return records internal cost without silently posting accounting',/expenseCreated:false/.test(read('equipmentCore'))&&/accountingPosted:false/.test(read('equipmentCore')));
...
check('visible navigation remains contract derived',read('productClient').includes('H38_UNIFIED=payload.unified')&&read('productClient').includes('renderNav();')&&contract.includes("label:'Today'")&&contract.includes("label:'Office'"));
...
check('equipment assignment does not silently contact customers or post accounting',direct.includes('No customer message or accounting post')&&portal.includes('No customer message or accounting post'));
check('equipment service does not silently create accounting entries',direct.includes('No expense or accounting entry is created')&&portal.includes('No expense or accounting entry is created'));
...
if(typeof runtime.boControlCapabilities_==='function'){
 ...
 const submitted=runtime.boSocialTransition_({status:'Draft'},'SUBMIT_REVIEW','Foreman','T1',true),approved=runtime.boSocialTransition_(submitted,'APPROVE','Owner','T2',true),ready=runtime.boSocialTransition_(approved,'PUBLISH','Owner','T3',true);
