...
const fs=require('fs');const [raw,result]=process.argv.slice(2);const text=fs.readFileSync(raw,'utf8').trim();if(!text)throw new Error('Apps Script execution returned no JSON.');const value=JSON.parse(text);if(value.error)throw new Error(`${value.error.message||'Apps Script execution failed'} ${JSON.stringify(value.error.details||[])}`);fs.writeFileSync(result,JSON.stringify(value.response,null,2)+'\n');
NODE
}
...
function boStandaloneBootstrap(){{const config=boStandaloneReadConfig_();const result=boBootstrapInstall(config);PropertiesService.getScriptProperties().setProperty('BUSINESS_OFFICE_BOOTSTRAP_CONFIG_FILE_ID',config.__configFileId);return result;}}
function boStandaloneSelfTest(){{return boRunSelfTest();}}
function boStandalonePlatformAcceptance(payload){{return boRunPlatformAcceptance(payload||{{}});}}
function boStandaloneRenderedHtml(){{return boGetRenderedWebAppHtml();}}
function boStandaloneCleanup(){{const id=PropertiesService.getScriptProperties().getProperty('BUSINESS_OFFICE_BOOTSTRAP_CONFIG_FILE_ID');if(id)DriveApp.getFileById(id).setTrashed(true);PropertiesService.getScriptProperties().deleteProperty('BUSINESS_OFFICE_BOOTSTRAP_CONFIG_FILE_ID');return {{status:'PASS',configurationDocumentTrashed:!!id}};}}
''')
PY
...
run_api boStandaloneBootstrap "$OUT/no-params.json" "$OUT/bootstrap-api.json" "$OUT/bootstrap-response.json"
run_api boStandaloneSelfTest "$OUT/no-params.json" "$OUT/self-test-api.json" "$OUT/self-test-response.json"
...
NODE
cp "$FIX"/* "$OUT/"
run_api boStandalonePlatformAcceptance "$FIX/platform-params.json" "$OUT/platform-acceptance-api.json" "$OUT/platform-acceptance-response.json"
...
run_api boStandaloneRenderedHtml "$OUT/no-params.json" "$OUT/rendered-ui-api.json" "$OUT/deployed-ui.json"
...
run_api boStandaloneCleanup "$OUT/no-params.json" "$OUT/cleanup-api.json" "$OUT/cleanup-response.json"
