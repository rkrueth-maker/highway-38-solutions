...
assert(loader.includes("tenant-attribution.js?build=20260926-tenant-attribution-mobile-framing-2"),'shared Office loader must load the current tenant attribution runtime');
...
assert(commercial.includes('max-width:min(210px,50vw)'),'shared mobile business branding must have usable phone width');
...
console.log(JSON.stringify({status:'PASS',acceptance:'HIGHWAY38_BRAND_ATTRIBUTION',northernPrimaryBrandPreserved:true,highway38Attribution:true,meetingReportsTenantAware:true,todayPromptTenantNeutral:true,quoteCredit:true,publicSiteCredit:true,customerPortalCredit:true,ownerAccessCredit:true,askH38Preserved:true,sharedMobileChrome:true,currentTenantRuntime:true},null,2));
