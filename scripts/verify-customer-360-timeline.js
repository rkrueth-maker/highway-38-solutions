...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('.h38-c360-workspace [data-c360-panel="overview"]').count(),1,'canonical Overview panel must own recent activity');
    assert.equal(await page.locator('[data-h38-c360-timeline]').count(),0,'legacy full timeline must not render above or beside the canonical workspace');
    const contract=await page.evaluate(()=>window.H38_CUSTOMER_360_TIMELINE);
    assert.equal(contract.chronologicalCustomerStory,true);
    assert.equal(contract.canonicalOverviewOwnsRecentActivity,true);
    assert.equal(contract.noStandaloneCanonicalTimeline,true);
    ...
    console.log(JSON.stringify({status:'PASS',checks:['canonical Overview owns recent activity','no standalone full timeline','customer safety boundaries']},null,2));
