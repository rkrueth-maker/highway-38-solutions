...
const direct=read(directFile),voice=read(voiceFile),operator=read(operatorFile),guard=read(guardFile),build=read(buildFile);
[ [directFile,direct],[voiceFile,voice],[operatorFile,operator],[guardFile,guard] ].forEach(([file,source])=>parse(file,source));
...
check(/versionCode\s+25\b/.test(build),'Owner APK versionCode must be 25.');
check(/versionName\s+'0\.5\.20'/.test(build),'Owner APK versionName must be 0.5.20.');

console.log('PASS field platform authority: one Android CameraX owner, iPhone native capture, direct quote/Site Visit deletes.');
