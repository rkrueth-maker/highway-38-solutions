...
function buildConcat(ids,outName){
...
const operatorMaster=buildConcat(operatorOrder,'00-NORTHERN-LAKES-OPERATOR-MASTER-TRAINING-NARRATED');
const referenceAppendix=buildConcat(referenceOrder,'99-NORTHERN-LAKES-REFERENCE-APPENDIX-NARRATED');
const manifest={title:'Northern Lakes Training',purpose:'Operator training in recommended learning order with static/reference material separated from the hands-on master',clockInOutNavigation:'On phone, Clock In / Out is an everyday action under the + quick-action menu. ERP is underlying Business Office architecture, not a peer operator action.',operatorOrder,referenceOrder,allParts:[...operatorOrder,...referenceOrder],operatorMaster:path.basename(operatorMaster),referenceAppendix:path.basename(referenceAppendix)};
...
console.log(JSON.stringify({status:'PASS',operatorMaster,operatorParts:operatorOrder.length,referenceAppendix,referenceParts:referenceOrder.length,totalParts:operatorOrder.length+referenceOrder.length},null,2));
