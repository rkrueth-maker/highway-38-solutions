...
(async()=>{
  ...
  try{
    ...
    await page.waitForSelector('[data-h38-c360-photo-stream]',{state:'attached'});
    assert.equal(await page.locator('[data-c360-panel="files"] > [data-h38-c360-photo-stream]').count(),1,'photo stream must live inside the canonical Files tab');
    assert.equal(await page.locator('.h38-c360-detail > [data-h38-c360-photo-stream]').count(),0,'photo stream must not sit between the customer header and tabs');
    await page.locator('[data-c360-tab="files"]').click();
    await page.waitForSelector('[data-h38-c360-photo-stream]',{state:'visible'});
    ...
    assert.equal(contract.canonicalFilesPanelOwned,true);
    assert.equal(contract.mutationObserverCleanupRemoved,true);
    console.log(JSON.stringify({status:'PASS',checks:['canonical Files-tab photo stream','no mutation observer cleanup','synced photo thumbnail','pending phone photo thumbnail','signed private preview','11 customer jobs visible','edit/delete on all rows','customer edit/delete controls','full-screen media viewer','no automatic customer release']},null,2));
