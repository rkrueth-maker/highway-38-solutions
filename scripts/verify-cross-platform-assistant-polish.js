...
async function verifyBrowser(browserType,name,userAgent,expectedClass){
  ...
  try{
    await page.setContent(`<!doctype html><html><head></head><body>
      ...
      </div></header><section class="business-bar"><span>Highway 38</span></section>
    ...
    return {name,topPadding};
  }finally{await browser.close();}
}

(async()=>{
  const android=await verifyBrowser(chromium,'Chromium Android shell','Mozilla/5.0 (Linux; Android 16) AppleWebKit/537.36 Chrome/151 Mobile Safari/537.36 H38SiteScannerAndroid/0.5.35','h38-native-android');
  const ios=await verifyBrowser(webkit,'WebKit iPhone web','Mozilla/5.0 (iPhone; CPU iPhone OS 18_6 like Mac OS X) AppleWebKit/605.1.15 Version/18.6 Mobile/15E148 Safari/604.1','h38-ios-like');
