...
syntax(liveClient,'closed-environment execution client');
syntax(liveServer,'closed-environment execution server');
...
check('Owner Portal closed-environment release is visible and Owner gated',h38Pack.workflow.closedEnvironment===true&&h38Pack.workflow.externalActionsEnabled===true&&h38Pack.workflow.ownerApprovalRequired===true&&h38Pack.workflow.selectedRecordOnly===false&&h38Pack.workflow.bulkExternalActionsEnabled===true&&h38Pack.workflow.automaticExternalTriggersEnabled===true&&ownerIndex.includes('Portal_LiveExternal_Client')&&liveClient.includes('CLOSED · OWNER AUTHORIZED')&&liveClient.includes('All implemented actions available')&&liveServer.includes('h38RequireLiveExternalAction_'));
...
check('Business Office closed-environment controls preserve authentication and evidence',officeServer.includes('externalActionsOccurred:false')&&liveServer.includes('ownerApprovalRequired:controls.ownerApprovalRequired')&&liveServer.includes('selectedRecordOnly:controls.selectedRecordOnly')&&liveServer.includes('bulkExternalActionsEnabled:controls.bulkExternalActionsEnabled')&&liveServer.includes('automaticExternalTriggersEnabled:controls.automaticExternalTriggersEnabled')&&h38Pack.boundaries.directPaymentProcessing===true&&h38Pack.boundaries.directPayrollFunding===true&&h38Pack.boundaries.directTaxFiling===true&&liveServer.includes('Proof Log, Audit, and Error Log remain active controls'));
const combined=ownerServer+'\n'+ownerUnified+'\n'+ownerClient+'\n'+officeServer+'\n'+officeUx;
const dangerous=['GmailApp.sendEmail','MailApp.sendEmail','ScriptApp.newTrigger','Stripe','PayPal'];
check('UX layer itself adds no uncontrolled provider execution',dangerous.every(token=>!combined.includes(token)),dangerous.filter(token=>combined.includes(token)).join(','));
...
run('scripts/verify-live-external-actions.js','closed-environment execution verification');
