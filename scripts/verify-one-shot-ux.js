...
need('index.html','class="site-header"','homepage uses real responsive header');
need('index.html','class="menu-button"','homepage provides accessible mobile navigation control');
...
need('index.html','assets/approved-homepage-mockup.png','homepage uses the exact approved PNG as a separate visual');
check('homepage removes invisible navigation hotspots',!home.includes('class="hotspot')&&!/hs-(?:logo|home|solutions|examples|proof|about|portal|request)/.test(home));
check('homepage removes horizontal swipe shell',!home.includes('approved-home__stage')&&!/Swipe horizontally/i.test(home));
check('homepage has four approved outcome paths',(home.match(/class="outcome-card"/g)||[]).length===4);
check('homepage preserves useful lower page sections',home.includes('Rough input to clear plan')&&home.includes('Practical experience')&&home.includes('Ready to start?'));
check('unsupported On Time On Point claim absent',!home.includes('On Time. On Point.'));
check('approved image remains separated from text and controls',home.includes('class="hero-copy"')&&home.includes('class="hero-media"'));
...
check('interactive controls meet minimum target',read('visual-cleanup.css').includes('min-height:44px')&&css.includes('min-height:44px'));
