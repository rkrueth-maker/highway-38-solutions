...
for(const s of ['automaticSavedOfficeFill:true','automaticSavedOfficeSignIn:true','Sign in with saved H38 Office login','Saved H38 Office login found. Signing in'])must(auth,s,'Office sign-in');
for(const s of ['./field-visit-server-recovery.js?build=20260811-server-draft-recovery-1158','./field-visit-guided-controller.js?build=20260811-guided-ar-advance-1','./field-visit-measurement-classification.js?build=20260810-measurement-classification-2140','./field-visit-video-measurements.js?build=20260811-video-headless-stable-1154','./field-visit-acceptance-fix.js?build=20260810-site-visit-acceptance-2230','./auth-autofill.js?build=20260910-office-access-1','./supabase-no-legacy-office.js?build=20260917-lifecycle-startup-stable-1','./supabase-runtime-globals.js?build=20260913-mobile-touch-targets-1'])must(index,s,'production runtime');
...
if(!/const CACHE_NAME='h38-business-office-\d{8}-(?:\d{4}|nav-core-\d+|physical-startup-\d+)'/.test(serviceWorker))throw new Error('startup service worker must use a dated, named, or physical-startup cache build');
