...
for (const item of packs) {
  ...
  if (item.name === 'highway38') {
    ...
    check(`${item.name} keeps external execution disabled`, !item.config.workflow || item.config.workflow.externalActionsEnabled === false);
...
check('Highway 38 closed execution cannot leak into template configuration', h38.workflow.closedEnvironment === true && template.workflow.externalActionsEnabled === false && template.tax.directFiling === false);
