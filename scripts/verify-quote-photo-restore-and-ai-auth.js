...
const cacheName = worker.match(/const CACHE_NAME='(h38-business-office-\d{8}-(?:\d{4}|nav-core-\d+|physical-startup-\d+))'/)?.[1];
if (!cacheName) fail('Service worker cache rotation must use an accepted dated, nav-core, or physical-startup h38-business-office cache name.');
