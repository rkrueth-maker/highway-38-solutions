...
async function selectJohnson(page){await page.waitForSelector('#h38Customer360Search',{state:'visible'});await page.waitForFunction(()=>window.H38_CUSTOMER_360?._renderPatched===true&&window.H38_CUSTOMER_360.searchCustomers?.(window.state?.snapshot||{},'Johnson')?.some?.(row=>row.customerId==='C-JOHN'),null,{timeout:10000});await page.evaluate(()=>{const input=document.getElementById('h38Customer360Search');input.value='Johnson';input.dispatchEvent(new Event('input',{bubbles:true}));});const result=page.locator('#h38Customer360Matches [data-c360-customer="C-JOHN"]').first();await result.waitFor({state:'attached',timeout:10000});assert.equal(await result.evaluate(n=>n.dataset.c360PolicyCustomer||n.dataset.c360Customer||''),'C-JOHN');await result.click({force:true});await page.waitForFunction(()=>window.H38_CUSTOMER_360?.selectedCustomerId==='C-JOHN'&&window.state?.page==='customers');assert.equal(await page.locator('#h38MeetingDialog[open]').count(),0,'customer selection must not open Meeting');}
