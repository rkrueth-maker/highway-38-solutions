...
function verifyArchitectureContract(){
  ...
  assert(mobile.includes("const PRIMARY_KEYS=['today','customers','schedule','messages']"),'mobile physical authority must recognize the customer-first primary order');
  assert(mobile.includes('jobsMovedToMore:true')&&mobile.includes('moreJobsNavigationFinalize:true'),'Jobs must stay stabilized when launched from grouped More');
...
async function selectJohnson(page){await page.waitForSelector('#h38Customer360Search',{state:'visible'});await page.waitForFunction(()=>window.H38_CUSTOMER_360?._renderPatched===true&&window.H38_CUSTOMER_360.searchCustomers?.(window.state?.snapshot||{},'Johnson')?.some?.(row=>row.customerId==='C-JOHN'),null,{timeout:10000});await page.evaluate(()=>{window.H38_CUSTOMER_360.selectedCustomerId='C-JOHN';window.H38_CUSTOMER_360.selectedTab='overview';window.renderCustomers?.();});await page.waitForFunction(()=>window.H38_CUSTOMER_360?.selectedCustomerId==='C-JOHN'&&window.state?.page==='customers'&&/Johnson/i.test(document.querySelector('.h38-c360-head h2')?.textContent||''),null,{timeout:10000});assert.equal(await page.locator('#h38MeetingDialog[open]').count(),0,'customer selection must not open Meeting');}
...
async function verifyMobile(phone){
  await phone.waitForFunction(()=>document.getElementById('mainNav')?.classList.contains('h38-five-primary-nav'),{timeout:5000});
  const labels=await phone.locator('#mainNav > button[data-h38-primary] span:last-child').allTextContents();
  assert.deepEqual(labels,['Today','Customers','Schedule','Messages','More'],'mobile primary order must be customer-first');
  assert.equal(await phone.locator('#mainNav > button[data-h38-primary="work"]').count(),0,'Jobs must not occupy the primary phone bar');
  assert.equal(await phone.locator('#mainNav > button[data-page="meetings"]').count(),0,'mobile must not grow sixth Meetings button');
  const customers=phone.locator('#mainNav > button[data-h38-primary="customers"]');assert.equal(await customers.count(),1);const hit=await customers.evaluate(n=>{const r=n.getBoundingClientRect(),t=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2)?.closest('button');return t?.dataset?.h38Primary||'';});assert.equal(hit,'customers','physical center of mobile Customers must hit Customers');await customers.click();await phone.waitForFunction(()=>window.state.page==='customers');await selectJohnson(phone);
  for(const key of ['today','customers','schedule','messages']){await phone.locator(`#mainNav > button[data-h38-primary="${key}"]`).click();await phone.waitForFunction(k=>window.state.page===k,key);assert.equal(await phone.locator('#mainNav > button[data-page="meetings"]').count(),0);await noDead(phone,`mobile ${key}`);}
  for(const key of ['work','quotes','documents','money']){await phone.locator('#mainNav > button[data-h38-primary="more"]').click();await phone.waitForSelector('#h38PrimaryMoreDialog[open]');const target=phone.locator(`#h38PrimaryMoreDialog [data-more-page="${key}"]`);assert.equal(await target.count(),1,`mobile More missing ${key}`);await target.click();await phone.waitForFunction(k=>window.state.page===k,key);assert.equal(await phone.locator('#mainNav > button[data-page="meetings"]').count(),0);await noDead(phone,`mobile ${key}`);}
  return {mobilePrimary:labels,mobileJobsInMore:true};
}
(async()=>{verifyOfficeSyntax();verifyArchitectureContract();const local=server();await new Promise(r=>local.listen(0,'127.0.0.1',r));const base=`http://127.0.0.1:${local.address().port}`;const browser=await chromium.launch({headless:true});try{
  ...
  const mobile=await browser.newContext({viewport:{width:390,height:844},serviceWorkers:'block'});await instrument(mobile);await stubCdn(mobile);const phone=await mobile.newPage();await boot(phone,base,snapshot('owner'));const mobileNav=await verifyMobile(phone);await mobile.close();
