...
function verifyArchitectureContract(){
  ...
  const cache=(worker.match(/const CACHE_NAME='(h38-business-office-\d{8}-(?:\d{4}|nav-core-\d+))'/)||[])[1]||'';
  assert(cache&&cache!=='h38-business-office-20260910-nav-core-4','navigation deployment must rotate service-worker cache');
}
