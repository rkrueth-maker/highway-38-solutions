...
for(const s of ['H38_ANDROID_NATIVE_WALKTHROUGH_GUARD','directNativeLaunch:true','mobileSaveAndStartOwned:true','mobileRecordAnotherOwned:true','privateCameraX:true','nativeChunkRecovery:true','noMediaStore:true','noWebRTCWalkthrough:true','persistVideoFirst','readNativeFile','confirmConsumed()'])must(nativeGuard,s,'native mobile guard');
for(const s of ['H38_FIELD_VISIT_FAST_FOLLOWUP','actualNotesOnCapture:true','nextPhotoImmediate:true','onePhotoAtATime:true','backgroundProcessing:true','workingHammer:true','NEXT PHOTO'])must(followup,s,'post-walkthrough follow-up');
...
for(const s of ['versionCode 27',"versionName '0.5.22'",'androidx.camera:camera-video:1.5.3'])must(gradle,s,'Android version');
...
console.log(JSON.stringify({status:'PASS',androidAuthority:'direct native CameraX bridge',iphoneAuthority:'native video input',sameVideoAudio:true,separateWebViewRecorder:false,durableNativeRecovery:true,workingHammer:true,actualNotesOnCapture:true,nextPhotoImmediate:true,backgroundProcessing:true,directQuoteDelete:true,directSiteVisitDelete:true,version:'0.5.22',automaticApproval:false,automaticCustomerSending:false},null,2));
