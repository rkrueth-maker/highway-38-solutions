...
for(const s of ['launchWalkthroughCapture','getRecoveredWalkthroughUrl','confirmRecoveredWalkthroughConsumed','CredentialManager','PasswordCredential','fillWebLogin'])must(bridge,s,'Android bridge');
for(const s of ['versionCode 31',"versionName '0.5.26'",'androidx.camera:camera-video:1.5.3'])must(gradle,s,'Android version');
...
console.log(JSON.stringify({status:'PASS',androidAuthority:'direct native CameraX bridge',iphoneAuthority:'native video input',sameVideoAudioDerivative:true,separateWebViewRecorder:false,durableNativeRecovery:true,visibleSwingHammer:true,editableSavedNotes:true,measurementVerificationFirst:true,plainVideoDepthGeometry:false,directQuoteDelete:true,directSiteVisitDelete:true,version:'0.5.26',automaticApproval:false,automaticCustomerSending:false},null,2));
