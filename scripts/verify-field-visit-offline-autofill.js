...
for(const s of ["email.name = 'username'","email.autocomplete = 'username'","password.autocomplete = 'current-password'",'requestAutofill','lastNativeRequest','Use saved username and password','h38:saved-login-filled'])must(auth,s,'autofill helper');
for(const s of ["CACHE_NAME='h38-business-office-20260809-1725'","FIELD_RECOVERY_BUILD='20260809-1725'",'android-native-walkthrough-guard.js','site-visit-top-action.js','auth-autofill.js'])must(worker,s,'service worker');
...
for(const s of ['setImportantForAutofill','AutofillManager','requestAutofill(webView)','H38SiteScannerAndroid/0.5.8','pendingFileCapture','WalkthroughCaptureActivity.class','Manifest.permission.CAMERA','Manifest.permission.RECORD_AUDIO','REQUEST_WALKTHROUGH_CAMERA_PERMISSION','pendingWalkthroughPermissionResume','requestPermissions(','launchWalkthroughVideoCapture','onRequestPermissionsResult','CAPTURE_RECOVERY_URL','persistCaptureTracking','restoreCaptureTracking','recoveredCaptureUri','openRecoveredWalkthroughResponse','confirmRecoveredWalkthroughConsumed','persistCaptureTracking(captured, true)','pendingFileCallback.onReceiveValue(new Uri[]{captured})'])must(main,s,'Android shell');
absent(main,'pendingFileCallback = null;\n                        clearCaptureTracking(false);','premature walkthrough recovery cleanup');
for(const s of ['PreviewView','CameraSelector.DEFAULT_BACK_CAMERA','VideoCapture.withOutput','withAudioEnabled()','FileOutputOptions','getFilesDir()','FileProvider.getUriForFile','Light On','Light Off','enableTorch','Stop & Use Video','WindowCompat.setDecorFitsSystemWindows','ViewCompat.setOnApplyWindowInsetsListener','WindowInsetsCompat.Type.systemBars()','dp(68)'])must(nativeCapture,s,'Android in-app walkthrough recorder');
...
for(const s of ['public void requestAutofill()','getRecoveredWalkthroughUrl','confirmRecoveredWalkthroughConsumed','CredentialManager','GetPasswordOption','PasswordCredential','fillWebLogin','h38:saved-login-filled'])must(bridge,s,'Android bridge');
for(const s of ["versionCode 13","versionName '0.5.8'","androidx.credentials:credentials:1.6.0-beta02","androidx.webkit:webkit:1.14.0","androidx.camera:camera-video:1.5.3","androidx.camera:camera-view:1.5.3"])must(gradle,s,'Android Gradle');
...
for(const f of [core,boot,video,ui,recovery,photoReview,voice,voiceCapture,owner,walkthroughGuidance,nativeGuard,topAction,auth])for(const s of ['automaticApproval:true','automaticCustomerSending:true','service_role','SUPABASE_SERVICE_ROLE_KEY'])absent(f,s,'field runtime');

console.log(JSON.stringify({status:'PASS',fieldVisitTabs:4,offlineFirst:true,existingSupabaseQueue:true,walkthroughFirst:true,durableVideoBeforeProcessing:true,targetedPhotosAfterWalkthrough:true,walkthroughVoiceTranscription:true,microphoneRequired:true,videoOnlyFallback:false,androidInAppVideoCapture:true,androidInAppMicrophone:true,androidTorchControl:true,androidCameraControlsInsetSafe:true,androidPrivateWalkthroughStorage:true,externalCameraHandoff:false,androidCameraPermissionResume:true,androidActivityRestartRecovery:true,androidRecoveredVideoIngest:true,androidRecoveryUntilAccepted:true,androidCredentialManagerLogin:true,androidWebRtcBypassedForInstalledShell:true,androidReturnToCaptureAfterVideo:true,androidNextStepFocused:true,siteVisitTopAction:true,keyboardSafeSiteVisitStart:true,automaticApproval:false,automaticCustomerSending:false},null,2));