...
for(const s of ['H38_ANDROID_NATIVE_WALKTHROUGH_GUARD','nativeEntryOnly:true','webrtcBypassed:true','saveAndStartGuarded:true','openNativeCapture','nextCaptureStep','returnToCapture:true','nextStepFocused:true','fieldPhotos'])must(nativeGuard,s,'native walkthrough guard');
...
for(const s of ["CACHE_NAME='h38-business-office-20260809-1518'","FIELD_RECOVERY_BUILD='20260809-1518'",'android-native-walkthrough-guard.js','site-visit-top-action.js'])must(worker,s,'service worker');
...
for(const s of ['setImportantForAutofill','AutofillManager','requestAutofill(webView)','H38SiteScannerAndroid/0.5.5','MediaStore.ACTION_VIDEO_CAPTURE','MediaStore.EXTRA_DURATION_LIMIT','MediaStore.EXTRA_OUTPUT','pendingFileCapture','pendingCaptureUri','createWalkthroughVideoUri','REQUEST_WALKTHROUGH_CAMERA_PERMISSION','pendingWalkthroughPermissionResume','requestPermissions(','launchWalkthroughVideoCapture','onRequestPermissionsResult'])must(main,s,'Android shell');
...
for(const s of ["versionCode 10","versionName '0.5.5'","androidx.credentials:credentials:1.6.0-beta02","androidx.webkit:webkit:1.14.0"])must(gradle,s,'Android Gradle');
for(const s of ['asset_statements','android:autoVerify="true"','highway38solutions.com','/commercial-app/','android.permission.CAMERA','android.permission.RECORD_AUDIO'])must(manifest,s,'Android manifest');
