...
for(const dir of [boDir,syncDir]){
  for(const file of fs.readdirSync(dir).filter(name=>name.endsWith('.gs'))){
    const source=path.join(dir,file),temp=path.join(process.cwd(),`.tmp-${dir===syncDir?'sync-':''}${file}.js`);
    fs.copyFileSync(source,temp);
    try{execFileSync(process.execPath,['--check',temp],{stdio:'pipe'});pass(`syntax ${dir===syncDir?'sync ':''}${file}`)}
    catch(error){fail(`syntax ${file}`,error.stderr?error.stderr.toString():error.message)}
...
assert('expected duplicate test error resolved',allSource.includes('Expected live-acceptance duplicate-protection result'));
