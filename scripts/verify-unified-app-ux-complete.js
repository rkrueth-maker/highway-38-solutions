...
check('legacy control and product clients are absent', !/(Portal_ControlPlane|Portal_ProductApps|Portal_ProductCenter|Portal_Product_Unification)/.test(index + raw));

const spaces = ['Today','Customers','Work','Money','Documents','Growth','Office'];
check('seven adaptive spaces are defined', spaces.every(label => registry.includes(`label:'${label}'`)), spaces.join(', '));
check('legacy Control space and Product Controls route are retired', !/id:'control'|label:'Control'|key:'bo:setup'/.test(registry));
...
check('owner home has a premium daily hero', /h38-owner-hero/.test(ownerHome) && /Owner access/.test(ownerHome) && /External actions/.test(ownerHome) && /Make decisions, answer customers/.test(ownerHome));
...
check('unified navigation applies field-role visibility', /h38FieldRoleKnown_/.test(manifest) && /h38FieldRoleCanView_/.test(manifest) && /bo:equipment/.test(registry));
...
parses('Portal_Module_Registry server', registry);
