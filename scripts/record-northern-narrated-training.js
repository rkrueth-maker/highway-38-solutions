...
function synth(text,file){
  ...
  const r=spawnSync('piper',['--model',piperModel,'--output_file',raw],{input:text+'\n',encoding:'utf8',maxBuffer:4*1024*1024});
  if(r.status!==0||!fs.existsSync(raw))throw new Error('Local narrator synthesis failed: '+clean(r.stderr||r.stdout));
  const f=spawnSync('ffmpeg',['-y','-i',raw,'-filter:a','atempo=0.88,volume=0.96',file],{encoding:'utf8',maxBuffer:8*1024*1024});
  try{fs.unlinkSync(raw);}catch(_){}
  if(f.status!==0||!fs.existsSync(file))throw new Error('Narrator pacing treatment failed: '+clean((f.stderr||f.stdout||'').slice(-1200)));
}
...
async function style(page){await page.addStyleTag({content:`#nlNarrationCaption{position:fixed;left:50%;bottom:14px;transform:translateX(-50%);z-index:2147483647;max-width:min(720px,calc(100vw - 32px));padding:7px 11px;border-radius:9px;background:rgba(9,31,43,.88);color:#fff;font:700 14px/1.3 system-ui;box-shadow:0 5px 18px rgba(0,0,0,.24);text-align:center;pointer-events:none;letter-spacing:.01em}@media(max-width:600px){#nlNarrationCaption{font-size:12px;bottom:78px;padding:6px 8px;max-width:calc(100vw - 28px)}}.nl-training-focus{outline:5px solid #ffbf47!important;outline-offset:4px!important;scroll-margin:110px!important}.nl-training-private{filter:blur(9px)!important;user-select:none!important}[href^="mailto:"],[href^="tel:"],input[type="email"],input[type="tel"]{filter:blur(8px)!important;user-select:none!important}`}).catch(()=>{});}
...
function captionText(text){const value=clean(text),first=(value.match(/^.*?[.!?](?:\s|$)/)||[value])[0].trim();return first.length<=88?first:first.slice(0,85).trimEnd()+'…';}
async function narrate(page,result,text){
  const i=result.narration.length,offsetMs=Math.max(0,Date.now()-result.recordingStartedMs),caption=captionText(text);await page.evaluate(t=>{let n=document.getElementById('nlNarrationCaption');if(!n){n=document.createElement('div');n.id='nlNarrationCaption';n.dataset.nlTrainingCaption='1';document.body.appendChild(n);}n.textContent=t;},caption);
  const wav=path.join(out,'audio',`${result.id}-${String(i+1).padStart(2,'0')}.wav`);synth(text,wav);const durationMs=probeMs(wav)||Math.max(2100,Math.ceil(text.split(/\s+/).length/1.95*1000));result.narration.push({text,caption,offsetMs,durationMs,wav});await page.waitForTimeout(durationMs+360);
}
...
async function record(browser,statePath,scenario){
  ...
  const result={id:scenario.id,title:scenario.title,tenant:'northern-lakes',viewport:mobile?'phone':'desktop',status:'HOLD',steps:[],narration:[],voiceProfile:'deep slow warm cinematic male system narrator',narrationRateTargetWpm:'125-145',externalActionsOccurred:false,startedAt:now(),recordingStartedMs:Date.now()};let raw='';
