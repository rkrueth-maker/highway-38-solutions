...
assert(bootstrap.includes("const PHONE_FIRST_BUILD='20260920-final-real-office-1'"),'phone-first build must be versioned in the live-first bootstrap');
...
(async()=>{
  ...
  try{
    ...
    await phone.waitForFunction(()=>window.H38_MOBILE_RUNTIME_STABILITY?.phoneFirstPrimaryNavigation===true&&window.H38_PHONE_FIRST_OFFICE?.build==='20260920-final-real-office-1');
    ...
    console.log(JSON.stringify({status:'PASS',build:'20260920-final-real-office-1',primary:labels,singleNavAuthority:true,customerSearch:true,simplifiedToday:true,groupedMore:true,siteVisitStepper:true,desktopUnchanged:true}));
