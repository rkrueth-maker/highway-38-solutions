...
check('same saved video is converted to a small voice derivative without opening another microphone',capture.includes('derivedSameVideoAudio:true')&&capture.includes('targetSampleRate:TARGET_RATE')&&capture.includes('Walkthrough Voice Audio')&&!capture.includes('navigator.mediaDevices')&&!capture.includes('new MediaRecorder'));
...
check('owner APK remains current runtime host',gradle.includes('versionCode 31')&&gradle.includes("versionName '0.5.26'"));
if(failures.length){console.error(JSON.stringify({status:'FAIL',failures},null,2));process.exit(1);}
console.log(JSON.stringify({status:'PASS',android:'CameraX auto-start only',iphone:'native video input',sameVideoAudioDerivative:true,nativeStreamingRecovery:true,automaticPostCaptureSync:true,automaticWalkthroughProcessing:true,automaticSavedOwnerFill:true,editableSavedNotes:true,measurementVerificationFirst:true,visibleSwingHammer:true,version:'0.5.26'},null,2));
