...
check('android host guarantees a return URI for saved walkthrough',android.includes('MediaStore.EXTRA_OUTPUT')&&android.includes('pendingCaptureUri')&&android.includes('createWalkthroughVideoUri'));
...
check('android version bumped',gradle.includes('versionCode 8')&&gradle.includes("versionName '0.5.3'")&&android.includes('H38SiteScannerAndroid/0.5.3'));
if(failures.length){console.error(JSON.stringify({status:'FAIL',failures},null,2));process.exit(1);}console.log(JSON.stringify({status:'PASS',checks:23},null,2));