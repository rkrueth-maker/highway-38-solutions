...
check('unrelated queue items do not block current walkthrough review',recovery.includes('processingWaitingOperations')&&recovery.includes('currentWalkthroughEvidenceGate:true')&&recovery.includes('unrelatedQueueDoesNotBlockReview:true'));
...
check('CameraX walkthrough ceiling is ten minutes',nativeCapture.includes('MAX_DURATION_MS = 600_000L')&&!nativeCapture.includes('MAX_DURATION_MS = 90_000L'));
...
console.log(JSON.stringify({status:'PASS',android:'CameraX auto-start only',iphone:'native video input',sameVideoAudioDerivative:true,nativeSameVideoAudio:true,nativeStreamingRecovery:true,currentWalkthroughEvidenceGate:true,unrelatedQueueDoesNotBlockReview:true,walkthroughSafetyCeilingMinutes:10,automaticPostCaptureSync:true,automaticWalkthroughProcessing:true,automaticSavedOwnerFill:true,editableSavedNotes:true,operatorVerifiedMeasurements:true,materialSpecificationsExcludedFromMeasurements:true,measurementVerificationFirst:true,oneGuidedController:true,walkthroughArAdvanceLinked:true,selectedTranscriptionSourceSizeGate:true,currentCandidateSource:'0.5.29',acceptedOwnerApkBaseline:'0.5.27',physicalPhoneAcceptanceExternal:true},null,2));