...
const logoPath='highway38-logo.png';
...
const index=read('commercial-app/index.html'),delivery=read('commercial-app/app-20.js'),deliveryCss=read('commercial-app/quote-delivery.css'),quoteWorkspace=read('commercial-app/app-07.js'),manifest=JSON.parse(read('commercial-app/manifest.webmanifest')),completionDefaults=read('apps-script/commercial-office-beta/CommercialBeta_CompletionCore_02.gs'),completionBootstrap=read('apps-script/commercial-office-beta/CommercialBeta_CompletionCore_04.gs'),scopedSnapshot=read('apps-script/commercial-office-beta/CommercialBeta_DeliveryAcceptance_01.gs'),web=read('apps-script/commercial-office-beta/CommercialBeta_Web.gs'),launcher=read('open-business-office.html'),handoff=read('apps-script/commercial-office-beta/CommercialBeta_Office.html'),setup=read('apps-script/commercial-office-beta/CommercialBeta_Setup.html'),live=read('scripts/verify-commercial-delivery-acceptance.js')+'\n'+read('scripts/verify-commercial-delivery-acceptance-v2.js'),workflow=read('.github/workflows/commercial-google-native-beta.yml');
...
const manifestIcon=(manifest.icons||[]).find(icon=>String(icon.src||'').split('?')[0]==='../assets/'+logoPath&&icon.type==='image/png'&&String(icon.purpose||'').includes('any'));
const openQuotePage=quoteWorkspace.indexOf("state.page='quotes'");
const openQuoteState=quoteWorkspace.indexOf('state.quote={quoteId:id',openQuotePage);
const openQuoteRender=quoteWorkspace.indexOf('renderQuotes();',openQuoteState);
const openQuoteReturn=quoteWorkspace.indexOf('return true;',openQuoteRender);
check('Office header approved logo',index.includes(`../assets/${logoPath}`)&&index.includes('id="approvedOfficeLogo"')&&index.includes('alt="Highway 38 Solutions"'));
check('Office installed icon approved logo',Boolean(manifestIcon));
check('secure launcher approved logo',launcher.includes(`assets/${logoPath}`)&&launcher.includes('id="approvedLauncherLogo"'));
check('authorized handoff approved logo',handoff.includes(`https://highway38solutions.com/assets/${logoPath}`)&&handoff.includes('id="approvedLogo"'));
check('setup approved logo',setup.includes(`https://highway38solutions.com/assets/${logoPath}`));
check('printable preview approved logo',delivery.includes(`const H38_APPROVED_LOGO='/assets/${logoPath}`)&&delivery.includes('class="quote-logo"')&&delivery.includes('Print / Save PDF'));
...
check('saved quote reopening stays in Quote Builder',openQuotePage>=0&&openQuoteState>openQuotePage&&openQuoteRender>openQuoteState&&openQuoteReturn>openQuoteRender);
...
check('delivery script isolates public and Office contexts',live.includes('verifyPublicWebsite(browser,config)')&&live.includes('verifyOfficeDelivery(browser,config,accessToken)')&&live.includes('isolatedBrowserContext:true'));
check('delivery script replays authorized fast startup before failing',live.includes('async function waitForFastOffice')&&live.includes('OFFICE_STARTUP_REPLAY')&&live.includes('OFFICE_STARTUP_RELOAD')&&live.includes('storedSession'));
check('delivery failure evidence preserves the live page',live.includes('failure-${safe}.png')&&live.includes('failure-${safe}.html')&&!live.includes('finally{await page.close();activePage=null;}'));
check('release workflow invokes delivery acceptance',workflow.includes('verify-commercial-delivery-acceptance.js'));
console.log(JSON.stringify({status:failures.length?'FAIL':'PASS',scopedSnapshotRequired:true,repeatedFullBootstrapAllowed:false,isolatedBrowserContextsRequired:true,authorizedStartupReplayRequired:true,checks,failures},null,2));
