...
(async()=>{
  ...
  await page.evaluate(()=>{
    ...
    window.H38_CUSTOMER_360={selectedCustomerId:'',selectedTab:'overview',customerBundle:()=>null,selectCustomer:function(customerId,tab='overview'){this.selectedCustomerId=customerId;this.selectedTab=tab;if(window.state.page!=='customers')window.openPage('customers');else window.renderCustomers?.();return true;}};
  });
  ...
  await page.evaluate(()=>setTimeout(()=>{window.H38_CUSTOMER_360.selectedCustomerId='C-LAWN';window.dispatchEvent(new Event('h38:business-snapshot-updated'));},250));
  await page.waitForTimeout(1700);
  assert.equal(await page.evaluate(()=>window.H38_CUSTOMER_360.selectedCustomerId),'C-NELSON','billing handoff must retain customer context through snapshot rerenders');
