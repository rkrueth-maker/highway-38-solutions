...
const manifest={installationId:pack.installationId,businessId:pack.business.id,businessName:pack.branding.businessName,mode,generatedAt:new Date().toISOString(),sourceFiles:sources.map(x=>x[0]),configurationHash:crypto.createHash('sha256').update(JSON.stringify(pack)).digest('hex'),apps:['quote-builder','customer-manager','work-manager','field-operations','document-center','invoice-payment-tracker','expense-receipt-manager','field-proof','social-control','customer-portal','request-intake-manager','price-book-template-manager','approval-center','vendor-purchase-manager','maintenance-manager','shop-flow-manager','business-system'],externalActionsEnabled:false,socialPublishingEnabled:false};
...
for (const required of ['BusinessOffice_ControlRules.gs','BusinessOffice_TaskCore.gs','BusinessOffice_ControlCore.gs','BusinessOffice_ControlLive.gs','BusinessOffice_ControlPlane.html']) if (!fs.existsSync(path.join(output,required))) throw new Error(`Generated installation is missing ${required}.`);
if (!generated.includes("key:'field-operations'") || !generated.includes("key:'social-control'")) throw new Error('Generated installation is missing focused Field Operations or Social Control apps.');
