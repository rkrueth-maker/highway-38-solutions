...
async function recordLifecycle(page,kind,result){
  ...
  await caption(page,'Enter the job name and scope before capturing site details.');
  await page.evaluate(async()=>{const f=document.getElementById('fieldContext'),w=window.H38_FIELD_VISIT_WORKFLOW;if(!f||!w)throw Error('Site Visit workflow unavailable.');await w.saveJobDraft(f,{announce:true});await w.ensureSession();window.H38_FIELD_VISIT_CORE.state.tab='capture';window.H38_FIELD_VISIT_CORE.state.render();});
  await page.locator('#fieldWalkthroughSkip').waitFor({timeout:10000});
  await caption(page,'For this training run, deliberately skip camera capture and continue with written evidence.');
  await page.locator('#fieldWalkthroughSkip').click();
  await page.locator('[data-tab="notes"]').click();
  await page.locator('#fieldNotes').fill('TEST visit complete. Access confirmed from the driveway. Verify final dimensions before work. Owner review required; nothing sent automatically.');
  await page.locator('#fieldSaveNotes').click();
  ...
  await caption(page,'Review the Site Visit before attaching it to the draft quote.');
  await page.locator('#fieldAttach').click();
  await page.locator('#quoteTitle').waitFor({timeout:20000});
