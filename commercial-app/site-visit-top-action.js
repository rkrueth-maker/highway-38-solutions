(function(){
'use strict';
const BUILD='20260816-native-save-start-launch-1';
...
function clickRealWalkthrough(attempt=0){const button=document.getElementById('fieldWalkthrough');if(button){button.click();return true;}if(attempt<4){requestAnimationFrame(()=>clickRealWalkthrough(attempt+1));return false;}try{window.toast?.('The walkthrough button is still loading. Tap Start Video Walkthrough once it appears.',true);}catch(_){}return false;}
async function nativeSaveAndStart(form){if(nativeSaveStartBusy)return;nativeSaveStartBusy=true;try{const workflow=window.H38_FIELD_VISIT_WORKFLOW,C=core();if(!workflow?.saveJobDraft||!workflow?.ensureSession||!C?.state)throw new Error('Site Visit is still loading.');await workflow.saveJobDraft(form);await workflow.ensureSession();C.state.tab='capture';await C.load?.();C.state.render?.();requestAnimationFrame(()=>clickRealWalkthrough());}catch(error){try{window.toast?.(error?.message||String(error),true);}catch(_){}}finally{nativeSaveStartBusy=false;}}
function interceptNativeSaveStart(event){if(!nativeAndroid())return;const form=event.target;if(!(form instanceof HTMLFormElement)||form.id!=='fieldContext')return;const submitter=event.submitter||document.activeElement;if(submitter?.id!=='fieldStartWalkthrough')return;event.preventDefault();event.stopImmediatePropagation();event.stopPropagation();void nativeSaveAndStart(form);}
function start(){
