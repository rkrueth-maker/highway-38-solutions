...
function client(){return db||(db=window.supabase.createClient(cfg.url,cfg.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'pkce'},global:{headers:{'x-client-info':'h38-supabase-quote-ai-v3'}}}));}
...
async function quoteAi(args,timeout,requestAction){
  ...
  try{
    ...
    const invokePromise=api.functions.invoke('h38-quote-ai',{
      ...
      headers:{authorization:`Bearer ${session.access_token}`,'x-client-info':'h38-supabase-quote-ai-v3'}
    });
...
Bridge.prototype.request=async function(action,args,timeout){
  if(action==='aiBuildQuoteDraft')return quoteAi(args||{},timeout,'buildQuote');
...
window.H38_SUPABASE_QUOTE_AI={enabled:true,endpoint:'h38-quote-ai',transport:'supabase-functions-invoke',authentication:'supabase-jwt',priceBookFirst:true,localResearchFallback:true,renderConcept:true,separateRenderRequest:true,ownerReviewRequired:true,automaticApproval:false,automaticSending:false};
})();