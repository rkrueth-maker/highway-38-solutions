...
function h38RetireLegacyNavigationArtifacts(){
  const legacyIds=['h38DesktopSidebarPhysicalProxy','h38DesktopNavHitLayerStyle','h38DesktopNavAuthority','h38DesktopNavHitLayer','h38DirectRouteProxyLayer','h38AuthCacheNavLayer','h38BusinessOfficeOpenHit','h38SiteVisitNativeHit','h38InboxControl'];
  legacyIds.forEach(id=>document.getElementById(id)?.remove());
  const nav=document.getElementById('mainNav');
  if(nav?.__h38DesktopNavClickHandler){try{nav.removeEventListener('click',nav.__h38DesktopNavClickHandler,true);}catch(_){}try{delete nav.__h38DesktopNavClickHandler;}catch(_){}}
  if(typeof window.h38DesktopNavWindowCapture==='function'){try{window.removeEventListener('click',window.h38DesktopNavWindowCapture,true);}catch(_){}}
}
...
loadCached=async function(options={}){
  ...
  const allowOnline=options?.allowOnline===true;if(navigator.onLine&&!allowOnline)return false;
  const userId=window.H38DB?.getUserScope?.()||'';if(!userId||!state.businessId)return false;
  const authorization=await get('meta','authorization');const checkedAt=new Date(authorization?.checkedAt||0).getTime();const maxAge=Number(window.H38_BUSINESS_OFFICE_SUPABASE?.offlineAuthorizationMaxAgeMs||0);const fresh=Number.isFinite(checkedAt)&&checkedAt>0&&maxAge>0&&Date.now()-checkedAt<=maxAge;
  ...
  const snapshot=await get('snapshots',`business:${state.businessId}`);if(!snapshot||snapshot.authUserId!==userId||snapshot.authorizationStatus!=='active'||snapshot.business?.businessId!==state.businessId)return false;
  const requestedKey=String(new URLSearchParams(location.search).get('businessKey')||'').trim().toLowerCase();if(requestedKey&&String(snapshot.business?.businessKey||'').toLowerCase()!==requestedKey)return false;
  const currentCheckedAt=new Date(state.snapshot?.authorizationCheckedAt||0).getTime();const cachedCheckedAt=new Date(snapshot.authorizationCheckedAt||snapshot.cachedAt||0).getTime();const currentIsNewer=state.snapshot?.authUserId===userId&&state.snapshot?.business?.businessId===state.businessId&&Number.isFinite(currentCheckedAt)&&currentCheckedAt>=cachedCheckedAt;
  if(!currentIsNewer){state.snapshot=snapshot;window.dispatchEvent(new CustomEvent('h38:business-snapshot-updated'));}
  $('businessStatus').textContent=navigator.onLine?`${state.snapshot.business.businessName} · Office open · refreshing securely…`:`${state.snapshot.business.businessName} · Offline · verified device cache ${new Date(state.snapshot.cachedAt||state.snapshot.authorizationCheckedAt).toLocaleString()}`;$('businessSelect').value=state.businessId;return true;
};
