async function loadBusiness(businessId,quiet=false){if(!businessId)return;state.businessId=businessId;await put('meta',{id:'selectedBusiness',businessId});try{const snapshot=await state.bridge.request('completionBootstrap',{businessId},90000);snapshot.id=`business:${businessId}`;snapshot.cachedAt=now();await put('snapshots',snapshot);state.snapshot=snapshot;$('businessSelect').value=businessId;$('businessStatus').textContent=`${snapshot.business.businessName} · ${snapshot.user.roleName} · cached ${new Date(snapshot.cachedAt).toLocaleTimeString()}`;renderNav();openPage(state.page,false);if(!quiet)toast('Business refreshed. Important records are now available offline.');return true;}catch(error){await loadCached();if(!quiet)toast(`${error.message} Using the most recent offline business pack.`,true);return false;}}
async function loadCached(){if(!state.businessId)return;const snapshot=await get('snapshots',`business:${state.businessId}`);if(snapshot){state.snapshot=snapshot;$('businessStatus').textContent=`${snapshot.business.businessName} · offline pack ${new Date(snapshot.cachedAt).toLocaleString()}`;$('businessSelect').value=state.businessId;renderNav();return true;}return false;}
function openPage(page,track=true){const pages=allowedPages();if(!pages.includes(page))page=pages[0]||'today';state.page=page;updateNavActiveState(page);renderPage();$('mainContent').focus({preventScroll:true});const renderedPage=page,renderedShell=state.shell;setTimeout(()=>{if(state.page!==renderedPage||state.shell!==renderedShell)return;window.dispatchEvent(new CustomEvent('h38:office-page-rendered',{detail:{page:renderedPage,shell:renderedShell}}));},0);if(track)recordUsage(page,'open-page').catch(()=>{});}
...
function renderMeetingsPage(){if(typeof window.renderMeetings==='function')return window.renderMeetings();$('mainContent').innerHTML=pageHead('Meetings','Meeting tools are loading. The main Business Office navigation remains available.')+`<section class="card"><p class="muted">Loading meeting history and capture tools…</p></section>`;setTimeout(()=>{if(state.page==='meetings'&&typeof window.renderMeetings==='function')window.renderMeetings();},100);}
function renderPage(){if(!state.snapshot){renderWelcome();return;}const renderers={today:renderToday,customers:renderCustomers,meetings:renderMeetingsPage,work:renderWork,quotes:renderQuotes,measure:renderMeasure,schedule:renderSchedule,messages:renderMessages,field:renderField,inventory:renderInventory,fleet:renderFleet,money:renderMoney,documents:renderDocuments,social:renderSocial,ai:renderAi,settings:renderSettings};(renderers[state.page]||renderToday)();}
