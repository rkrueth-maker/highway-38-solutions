(function(){
'use strict';
const BUILD='20260811-guided-photo-camera-1';
const C=window.H38_FIELD_VISIT_CORE;
if(!C)return;
let busy=false,stream=null,overlay=null;
...
async function saveFrame(video,button){
  ...
  try{
    ...
    closeCamera();
    C.toast('Requested photo saved. H38 is reviewing it.');
    if(navigator.onLine)setTimeout(()=>void window.H38_FIELD_VISIT_GUIDANCE?.reanalyze?.(),250);
  }catch(error){
...
window.H38_GUIDED_PHOTO_CAMERA={build:BUILD,close:closeCamera,androidRearCamera:true,savesThroughFieldVisitCore:true};
})();
