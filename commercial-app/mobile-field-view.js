(function(){
'use strict';
const BUILD='20260918-one-shell-field-experience-2';
...
function renderMyDay(){
  ...
  let host=document.getElementById(MY_DAY_ID);
  ...
  const signature=JSON.stringify({job:id(job,'Job ID','jobId'),jobStatus:text(value(job,'Status','status')),task:id(task,'Task ID','taskId'),taskStatus:text(value(task,'Status','status')),time:id(ctx.time,'Time Entry ID','timeEntryId'),proof:proof.items,next:id(next?.row,'Schedule Event ID','scheduleEventId'),nextStart:next?.start?.toISOString?.()||'',remaining:remaining.map(item=>id(item.row,'Schedule Event ID','scheduleEventId')),online:navigator.onLine!==false});
  if(host?.dataset.h38FieldSignature===signature)return;
  if(host)host.remove();host=document.createElement('section');host.id=MY_DAY_ID;host.className='h38-field-my-day';host.dataset.h38FieldSignature=signature;
...
function renderJobHub(){
  ...
  const signature=JSON.stringify({jid,jobStatus:text(value(job,'Status','status')),task:id(task,'Task ID','taskId'),taskStatus:text(value(task,'Status','status')),time:id(time,'Time Entry ID','timeEntryId'),visit:id(visit,'Capture Session ID','captureSessionId','Site Visit ID','siteVisitId'),visitStatus:text(value(visit,'Status','status')),proof:proof.items,files:files.map(row=>[id(row,'Document ID','documentId'),text(value(row,'Updated Time','Created Time'))]),activity:activity.map(row=>[id(row,'Job Note ID','Daily Log ID','Capture Session ID','Site Visit ID','id'),text(value(row,'Updated Time','Created Time'))])});
  if(host?.dataset.h38FieldSignature===signature)return;
  if(host)host.remove();host=document.createElement('section');host.id=JOB_HUB_ID;host.className='h38-field-job-hub';host.dataset.h38FieldSignature=signature;
