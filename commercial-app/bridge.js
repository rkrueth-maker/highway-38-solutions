class H38Bridge{
  constructor(frame,url,onStatus,onBootstrap,onFullSnapshot,onError){
    ...
    this.relay='BroadcastChannel' in window?new BroadcastChannel(this.channelName):null;
  ...
  receive(event){
    ...
    if(event.source===this.frame.contentWindow)source='frame';
  ...
  request(action,args={},timeout=45000){
    ...
    return new Promise((resolve,reject)=>{
      ...
      if(!this.ready||!target){clearTimeout(timer);this.pending.delete(requestId);reject(new Error('Secure Google connection is not ready. Tap Sign in securely, then try again.'));return;}
