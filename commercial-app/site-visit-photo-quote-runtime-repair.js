(function(){
'use strict';
const BUILD='20260821-site-visit-photo-quote-runtime-repair-3';
...
let evidenceBusy=null,decorateTimer=0,quoteBuildBusy=null,lastQuoteResult=null;
...
function buildPolicy(){return[
  ...
  'System quote policy and reusable examples are rules only. They never add a trade, material, or work item to the requested project scope.',
  'Separate material and labor into distinct owner-review lines when both are genuinely part of an explicit work item. Trade-specific examples apply only when that trade is actually named in the project title or scope.',
  ...
  'Search the supplied Price Book first. If no safe current same-unit catalog rate exists, leave the rate owner-review/manual-required instead of blocking the editable draft or pretending current research occurred.',
  'When an exact dimension or count is missing, do not invent it. A clearly labeled quantity-1 lump-sum/job planning allowance may be used for an explicitly requested work item, while the exact unknown remains in missingInformation.',
...
function requestId(){try{return crypto.randomUUID();}catch(_){return `REQ-${Date.now()}-${Math.random().toString(16).slice(2)}`;}}
function preparedArgs(args,evidence){const prepared={...(args||{})},ownerWorkRequest=cleanOwnerNotes(prepared.notes);prepared.notes=ownerWorkRequest;prepared.ownerWorkRequest=ownerWorkRequest;prepared.systemQuotePolicy=buildPolicy();prepared.clientRuntimeBuild=BUILD;prepared.requestId=requestId();prepared.materialOrderAllowancePercent=10;prepared.separateMaterialAndLabor=true;if(evidence.length){prepared.measurementEvidence=evidence;prepared.siteMeasurements=evidence;}return prepared;}
...
function draftProblems(payload){const lines=draftLines(payload),problems=[];if(!lines.length)problems.push('no quote lines were returned');const badQty=lines.filter(line=>!(Number(line?.quantity)>0)),badRate=lines.filter(line=>!(Number(line?.rate??line?.unitPrice)>0)&&text(line?.priceSource).toLowerCase()!=='manual_required');if(badQty.length)problems.push(`non-positive quantities: ${badQty.slice(0,5).map(line=>text(line?.description)).join('; ')}`);if(badRate.length)problems.push(`non-positive non-manual rates: ${badRate.slice(0,5).map(line=>text(line?.description)).join('; ')}`);return problems;}
function editableDraft(payload,args){payload=payload&&typeof payload==='object'?payload:{};payload.draft=payload.draft&&typeof payload.draft==='object'?payload.draft:{};let lines=draftLines(payload);const warnings=[];if(!lines.length){lines=[{description:text(args?.projectTitle||'Project work')||'Project work',costType:'other',quantity:1,unit:'lump sum',rate:0,catalogId:'',priceSource:'manual_required',confidence:'low',rationale:'Editable owner-review placeholder because AI returned no safe line breakdown. Needs pricing and quantity review; no customer commitment was made.'}];warnings.push('AI did not return a safe line breakdown; an editable owner-review placeholder was created.');}
  lines=lines.map(line=>{const next={...(line||{})};if(!(Number(next.quantity)>0)){next.quantity=1;next.unit=text(next.unit)||'lump sum';next.confidence='low';next.rationale=`${text(next.rationale)} Provisional owner-review quantity of 1 keeps the draft editable; exact quantity still needs confirmation.`.trim();warnings.push(`Needs quantity: ${text(next.description)||'quote line'}`);}if(!(Number(next.rate??next.unitPrice)>0)){next.rate=0;next.catalogId='';next.priceSource='manual_required';next.confidence='low';next.rationale=`${text(next.rationale)} Needs pricing: no safe approved/current rate was available, so the editable draft remains owner-review required.`.trim();warnings.push(`Needs pricing: ${text(next.description)||'quote line'}`);}return next;});
  payload.draft.suggestedLines=lines;payload.draft.missingInformation=unique([...(Array.isArray(payload.draft.missingInformation)?payload.draft.missingInformation:[]),...warnings]);payload.editableDraft=true;payload.ownerReviewWarnings=warnings;return payload;}
function quoteKey(args){return [text(args?.businessId),text(args?.quoteId),text(args?.projectTitle),text(args?.scope)].join('|');}
async function buildQuote(args,timeout){const key=quoteKey(args);if(quoteBuildBusy)return quoteBuildBusy;if(lastQuoteResult&&lastQuoteResult.key===key&&Date.now()-lastQuoteResult.at<120000){if(lastQuoteResult.error)throw lastQuoteResult.error;return lastQuoteResult.payload;}
  quoteBuildBusy=(async()=>{try{if(typeof window.sync==='function')await window.sync(false);}catch(_){}await hydrateEvidence('quote-build');let auth=await validatedSession(false),evidence=await liveMeasurements(auth,args),prepared=preparedArgs(args,evidence),payload=await quoteAttempt(prepared,timeout),problems=draftProblems(payload);if(problems.length){prepared={...prepared,requestId:requestId(),systemQuotePolicy:[prepared.systemQuotePolicy,'OWNER DRAFT REPAIR: The prior response did not produce a fully usable estimate. Preserve the explicit project title/scope and verified evidence. Correct line structure where possible, but do not invent geometry or block the editable draft just because a rate or quantity needs owner review.',`PROBLEMS TO CORRECT: ${problems.join(' | ')}`].join('\n\n')};payload=await quoteAttempt(prepared,timeout);}payload=editableDraft(payload,args);payload.h38FinalPhotoQuoteRepair=true;payload.h38LiveMeasurementCount=evidence.length;payload.h38RequestId=prepared.requestId;payload.h38ClientRuntimeBuild=BUILD;lastQuoteResult={key,at:Date.now(),payload,error:null};return payload;})().catch(error=>{const err=error instanceof Error?error:Error(text(error));lastQuoteResult={key,at:Date.now(),payload:null,error:err};throw err;}).finally(()=>{quoteBuildBusy=null;});return quoteBuildBusy;}
...
document.addEventListener('click',event=>{const button=event.target?.closest?.('button,[role="button"]');if(button&&/(?:build|refresh|retry).*(?:quote|draft)|(?:quote|draft).*(?:build|refresh|retry)|finish.*build/i.test(text(button.textContent)))lastQuoteResult=null;if(event.target?.closest?.('[data-make-action-picture],#fieldAttach'))setTimeout(()=>void hydrateEvidence('owner-action'),80);},true);
