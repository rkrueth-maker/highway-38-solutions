(function(){
'use strict';
const BUILD='20260821-site-visit-photo-quote-runtime-repair-1';
...
async function persistPointer(auth,v,selected,sessionRow,quoteRow){
  if(!selected)return;
  const stamp=now(),bid=businessId(v),qid=quoteId(v);
  if(sessionRow){const p=sessionRow.payload||{},next={...p,'Action Picture ID':selected,'Site Visit ID':text(v.visitId),'Capture Session ID':text(v.sessionId),'Updated Time':stamp,'Record Version':Number(value(p,'Record Version','recordVersion')||1)+1};const changed=await auth.client.from('business_records').update({payload:next,updated_by:auth.user.id,updated_at:stamp}).eq('business_id',bid).eq('collection','siteCaptureSessions').eq('record_key',sessionRow.record_key).eq('record_status','active');if(changed.error)throw changed.error;sessionRow.payload=next;updateSnapshot('siteCaptureSessions',v.sessionId,{'Action Picture ID':selected,'Site Visit ID':text(v.visitId),'Updated Time':stamp});}
  if(quoteRow&&qid){const p=quoteRow.payload||{},next={...p,'Action Picture ID':selected,'Site Visit ID':text(v.visitId),'Site Scanner Session ID':text(v.sessionId),'Updated Time':stamp,'Record Version':Number(value(p,'Record Version','recordVersion')||1)+1};const changed=await auth.client.from('business_records').update({payload:next,updated_by:auth.user.id,updated_at:stamp}).eq('business_id',bid).eq('collection','quotes').eq('record_key',quoteRow.record_key).eq('record_status','active');if(changed.error)throw changed.error;quoteRow.payload=next;const rows=window.state?.snapshot?.quotes;if(Array.isArray(rows)){const row=rows.find(item=>text(value(item,'Quote ID','quoteId'))===qid);if(row)Object.assign(row,{'Action Picture ID':selected,'Site Visit ID':text(v.visitId),'Site Scanner Session ID':text(v.sessionId),'Updated Time':stamp});}}
}
async function ensureQuoteLink(auth,v,source){
  const qid=quoteId(v),bid=businessId(v),original=documentId(source);if(!qid||!original)return'';
  const p=source.payload||{},suffix=safe(qid.replace(/^QUOTE-/i,'')).slice(0,8),recordKey=`QUOTE-LINK-${safe(original)}-${suffix}`.slice(0,220),stamp=now();
  const payload={...p,'Document ID':recordKey,'Source Type':'Quote','Source ID':qid,'Quote ID':qid,'Original Document ID':original,'Linked Site Visit ID':text(v.visitId),'Site Visit ID':text(v.visitId),'Capture Session ID':text(v.sessionId),'Action Picture':true,'Action Picture Source ID':original,'Action Picture Selected Time':stamp,'Customer Quote Selected':false,'Visibility':'Internal Action Picture','Access Classification':'Internal','Updated Time':stamp};
  const upsert=await auth.client.from('business_records').upsert({business_id:bid,collection:'documents',record_key:recordKey,payload,record_status:'active',created_by:auth.user.id,updated_by:auth.user.id,updated_at:stamp},{onConflict:'business_id,collection,record_key'});if(upsert.error)throw upsert.error;
  const map=window.H38_QUOTE_ACTION_PHOTO_BY_QUOTE||(window.H38_QUOTE_ACTION_PHOTO_BY_QUOTE=Object.create(null));map[qid]=original;return recordKey;
}
async function hydrateEvidence(trigger='runtime'){
  ...
  evidenceBusy=(async()=>{
    ...
    const images=(docsResult.data||[]).filter(row=>siteImage(row,v)),ids=unique(images.map(documentId));images.forEach(row=>serverImages.set(documentId(row),row));
    ...
    if(selected){const source=images.find(row=>documentId(row)===selected);if(source){await persistPointer(auth,v,selected,sessionRow,quoteRow);await ensureQuoteLink(auth,v,source);}}
