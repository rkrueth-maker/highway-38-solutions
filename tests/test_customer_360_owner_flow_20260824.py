...
BROWSER=(ROOT/'commercial-app/customer-360-browser-integration.js').read_text()
...
def test_assistant_is_customer_first_without_rewriting_personal_assistant():
    assert 'H38_CUSTOMER_360' in BROWSER
    assert 'resolveAssistantQuery' in BROWSER
...
def test_source_only_children_get_unique_customer_hint_but_conflicts_do_not_guess():
...
def test_customer_360_dynamic_loader_is_loaded_by_office_shell():
    assert 'customer-360-browser-integration.js?build=20260824-customer-360-browser-integration-1' in INDEX
