...
def test_final_attach_requires_exact_restored_visit_and_session():
    ...
    assert "item?.businessId" in FINAL
...
def test_final_attach_is_single_video_recovery_authority():
    assert "singleVideoAttachAuthority:true" in FINAL
    assert "legacyGuardBypassed:true" in FINAL
    assert "H38_ANDROID_NATIVE_WALKTHROUGH_GUARD" not in FINAL
    assert "h38:native-return-site-restored" in FINAL
...
def test_native_video_is_not_consumed_until_attachment_and_exact_draft_are_verified():
    ...
    attach = FINAL.split("async function attach", 1)[1].split("function schedule", 1)[0]
    ...
    assert attach.index("if(await durableVideoAlreadyAttached(item,visit))") < attach.index("confirmConsumed()")
    new_file_path = attach.split("const file=await readNativeFile()", 1)[1]
