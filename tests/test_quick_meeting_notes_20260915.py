...
RUNTIME=(ROOT/'commercial-app/quick-meeting-notes.js').read_text()
...
def test_quick_meeting_is_notes_only_and_customer_optional():
    for marker in ['customerOptional:true','temporaryAudioOnly:true','recordingPersisted:false','transcriptPersisted:false','meetingReportDocument:false',"storagePolicy:'NOTES_ONLY'",'noteSections:notes']:
...
def test_quick_meeting_has_one_simple_phone_entry_and_customer_layout_repair():
    for marker in ['h38QuickMeetingTodayCard','Start meeting','h38-qm-customer-clean','Start Follow-up Meeting'.lower(),"office().page!=='customers'"]:
        assert marker in RUNTIME
...
def test_runtime_is_live_first_and_bootstrapped():
    assert 'quick-meeting-notes.js?build=' in BOOT
