...
def test_final_authorities_are_loaded_directly_from_index():
    expected = [
        ...
        './field-visit-quote-handoff.js?build=20260922-site-visit-quote-customer-authority-7',
    ...
    assert "explicitCustomerId=text(C.state.visit?.customerId)" in handoff
    assert "openDraftQuote(quoteId,explicitCustomerId)" in handoff
    assert "enforceQuoteCustomer(qid,cid)" in handoff
