...
def test_native_save_start_is_bounded_at_every_pre_camera_stage():
    assert "SAVE_TIMEOUT_MS=4500" in FINAL
    assert "SESSION_TIMEOUT_MS=3500" in FINAL
    assert "BRIDGE_TIMEOUT_MS=1800" in FINAL
    assert "bounded(()=>workflow.saveJobDraft(form),SAVE_TIMEOUT_MS" in FINAL
    assert "bounded(()=>workflow.ensureSession(),SESSION_TIMEOUT_MS" in FINAL
    assert "indefiniteHammer:false" in FINAL
    assert "H38_NATIVE_SAVE_START_TIMEOUT" in FINAL


def test_native_save_start_preserves_return_context_before_direct_bridge_launch():
