...
This is the mandatory intake, ownership, implementation, deletion, performance, and release policy for every public website and authenticated web-application change in this repository.

It applies to every chat, agent, branch, pull request, direct commit, workflow, automation, reusable package, customer installation, public page, private portal, and module. A narrower handoff may add restrictions but may not weaken this policy.
...
8. keep startup at one browser-to-server RPC;
9. preserve existing Apps Script project IDs, deployment IDs, URLs, records, proof, errors, approvals, permissions, backups, audit history, and customer isolation;
...
12. include migration/default behavior for new fields and rollback evidence for schema changes;
...
- Shared website navigation/footer behavior changes only in the canonical public shell.
...
- Shared web-app shell/startup behavior changes only in the product client and unified bootstrap.
...
- Secondary/admin modules: on-demand loading.
- Public shell: one canonical JavaScript file and one canonical shell stylesheet.
- Request page: focused options controller and one secure submit owner.
- No duplicate bootstrap, schema, role, navigation, module-list, or saved-view calls.
- No page-wide DOM observers.
- No runtime image insertion or source swapping.
- Below-fold images use lazy loading and stable dimensions/aspect ratios.
- Shared changes replace duplicate code rather than adding another override.
- Performance verifiers may not be weakened to permit slower or duplicated architecture.
...
Do not weaken or bypass a failing verifier. Correct the source or correct a stale verifier so it validates the canonical architecture.
...
- Record the exact source commit, workflow run, conclusion, deployed SHA/version, and live cache-busted verification.
- Never create another Apps Script project or production deployment to avoid fixing the existing one.

## 11. Definition of done

A website or web-app addition/change is done only when:
...
- performance budgets remain intact;
- obsolete code and routes are removed or redirected;
- governance and domain-specific verification pass;
