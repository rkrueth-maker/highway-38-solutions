...
for(const marker of ['mobile-ux-v6','Use phone location','Search results','Opening Resale does not start a scan','Show ','h38-resale-location-v1','CANDIDATE · NEEDS SOLD COMPS','LOCATION NEEDS PROOF','completed with no current verified or candidate results'])if(!resale.html.includes(marker))throw Error('Resale missing '+marker);
...
vm.runInContext(rowsFn[0]+";result={deals:rows({opportunities:[],candidates:[{title:'candidate'}]},'deals'),facebook:rows({results:[],candidates:[{title:'listing'}]},'facebook'),stores:rows({readings:[{title:'price'}]},'stores')};",rowSandbox);
