...
for(const marker of ['data-h38-resale-web=','Use phone location','Loading saved H38 buy leads','Show ','h38-resale-location-v1','CANDIDATE · NEEDS SOLD COMPS','LOCATION NEEDS PROOF','Nearby Stores','NEARBY STORE','completed with no current verified or candidate'])if(!resale.html.includes(marker))throw Error('Resale missing '+marker);
for(const marker of ['All Opportunities','Repairable','Motivated Seller','Underpriced','PROFIT NOT VERIFIED','Screening spread $','NOT PROFIT','profit_verified===true',"q+' needs work'","q+' OBO'",'h38-shopping-location-v1','Deal ready to evaluate','Build coupon stack','not proof that no resale opportunities exist'])if(!resale.html.includes(marker))throw Error('Opportunity Hunter / Connected Resale UX missing '+marker);
for(const marker of ['source-strip','When:','Where:','Unknown pickup distance','Date needs proof'])if(!resale.html.includes(marker))throw Error('Garage/Estate truth UX missing '+marker);
...
for(const marker of ['data-h38-coupon-web=','Optimize my list','max_stores:max','Delete price','Remove watch','Delete receipt','Scanned list added'])if(!coupon.html.includes(marker))throw Error('Couponing missing '+marker);
