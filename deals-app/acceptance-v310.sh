...
grep -Fq 'H38_DEALS_THIN_SHELL_V311' "$ROOT/native/h38-site-scanner/android-app/reseller/src/main/java/com/highway38/resellerscout/MainActivity.java"
...
grep -Fq 'AndroidH38Deals' "$ROOT/native/h38-site-scanner/android-app/reseller/src/main/java/com/highway38/resellerscout/MainActivity.java"
grep -Fq 'speakList' "$ROOT/native/h38-site-scanner/android-app/reseller/src/main/java/com/highway38/resellerscout/MainActivity.java"
! grep -Fq 'bundledPage' "$ROOT/native/h38-site-scanner/android-app/reseller/src/main/java/com/highway38/resellerscout/MainActivity.java"
! grep -Fq 'FacebookMarketplaceActivity' "$ROOT/native/h38-site-scanner/android-app/reseller/src/main/AndroidManifest.xml"
...
cleanup_coupon_ci(){
  ...
  for pair in "coupon_watch_rules:$watch_id" "coupon_receipts:$receipt_id" "coupon_price_observations:$price_id" "coupon_list_items:$item_id" "coupon_shopping_lists:$list_id"; do
    table="${pair%%:*}"; id="${pair#*:}"
    if [ -n "$id" ]; then
      curl -sS -X DELETE "$SB_URL/rest/v1/$table?id=eq.$id" -H "apikey: $SB_KEY" -H "Authorization: Bearer $token" >/dev/null
    fi
  done
}
...
curl -fsS "$SB_URL/rest/v1/coupon_list_items?id=eq.$item_id&select=id,item_name" -H "apikey: $SB_KEY" -H "Authorization: Bearer $token" | jq -e 'length==1'

call_coupon(){
  local name="$1" body="$2"
  curl --max-time 45 -fsS -X POST "$SB_URL/functions/v1/h38-coupon-api" -H "apikey: $SB_KEY" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' --data "$body" | tee "$REPORT/coupon-$name.json"
}
...
echo H38_DEALS_LIVE_ACCEPTANCE_PASS | tee "$REPORT/live-acceptance.txt"
