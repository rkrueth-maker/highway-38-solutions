...
COLLECTIONS = {
    ...
    "Storage Solutions": {
        "description": "Strong garage storage starts with products that can handle weight, clutter, and daily use. This collection includes shelving and storage products for home garages, workshops, and utility spaces where tools, supplies, and seasonal items need a dependable place to go.",
    ...
    "Automotive Essentials": {
        "description": "Keep your vehicle, garage, and daily driving setup more organized and protected with automotive essentials selected for practical use.",
        "products": [
...
def find_collection_by_title(title):
    ...
    """
    data = client.graphql(query, {"query": f"title:'{title}'"})
...
def create_collection(title, description):
    ...
    """
    data = client.graphql(mutation, {"input": {"title": title, "descriptionHtml": f"<p>{description}</p>"}})
...
def find_product_by_title_keyword(keyword):
    ...
    data = client.graphql(query, {"query": keyword})
...
def add_products_to_collection(collection_id, product_ids):
    ...
    """
    data = client.graphql(mutation, {"id": collection_id, "productIds": product_ids})
