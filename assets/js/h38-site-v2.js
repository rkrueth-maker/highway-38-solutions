(()=>{
  ...
  if(!ROOT_PAGE)return;
  const VERSION='2026-07-27-quote-builder-examples';
  const LOGO='assets/highway38-logo.png?v=20260720-exact-0cbc4514';
  const registry={
    version:VERSION,
    logo:LOGO,
    navigation:[
      {href:'quote-builder.html',label:'Quote Builder'},
    ...
    footer:[
      {heading:'Start',links:[['Quote Builder','quote-builder.html'],['Start a Project','start-request.html'],['Pricing','pricing.html']]},
  ...
  function mount(){if(registry.mounted)return;ensureStyles();ensureSkipLink();mountHeader();mountFooter();wireMenu();routeExampleLinks();routeOwnerLinks();optimizeImages();registry.mounted=true;document.documentElement.dataset.h38SiteVersion=VERSION;}
