(()=>{
  ...
  function renderOfferNote(){if(!note)return;const item=selectedOffer();if(!item){note.textContent=pricing.aiPolicy;return;}const extra=item.id==='business-snapshot'?item.creditPolicy:pricing.aiPolicy;note.innerHTML=`<strong>${item.name}: ${item.priceLabel}</strong>${item.setupLabel?` · ${item.setupLabel}`:''}<br>${extra}`;}
  function selectOfferForOutcome(force){if(!offerSelect)return;const suggested=outcomeDefault[outcome?.value]||'';if(suggested&&(force||!offerSelect.value))offerSelect.value=suggested;renderOfferNote();}
  ...
  function summaryText(){const item=selectedOffer();return ['HIGHWAY 38 REQUEST SUMMARY',`Result needed: ${selectedText('outcome')||'Owner review required'}`,`Selected offer: ${item?offerLabel(item):'Recommend the right path'}`,`Name: ${fieldValue('name')}`,`Email: ${fieldValue('email')}`,`Phone: ${fieldValue('phone')}`,`Preferred contact: ${selectedText('contact')}`,`Current problem: ${fieldValue('problem')}`,`Finished result: ${fieldValue('desired')}`,`Timing: ${selectedText('timing')}`,`Implementation or setup budget: ${selectedText('budget')}`,`Files or links: ${fieldValue('files')}`,`Details and constraints: ${fieldValue('details')}`,'',pricing.aiPolicy,'Submitting creates a secure request for owner review. It creates no charge, subscription, purchase, payment, quote send, work start, or other automatic external action.'].join('\n');}
