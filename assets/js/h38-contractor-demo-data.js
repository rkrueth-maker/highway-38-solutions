window.H38ContractorDemo = Object.freeze({
  ...
  imageUrl(id) { return `https://drive.google.com/thumbnail?id=${id}&sz=w1600`; },
  quotes: {
    flower: {
      ...
      jobInstructions: ["Confirm the approved garden outline, utility locations, access route, and final plant selections with the owner.","Protect the house, lawn, driveway, and existing landscape features before work begins.","Lay out and mark the approximately 240 sq ft bed and verify finished elevations and drainage direction.","Remove sod and unwanted vegetation, then prepare and level the planting bed.","Place and blend the topsoil and amendment allowance; fine-grade the bed for planting.","Install edging, plants, and mulch to the approved layout and spacing.","Water in the installation, restore disturbed edges, remove debris, photograph the finished work, and complete owner walkthrough."]
    },
    drive: {
      ...
      jobInstructions: ["Confirm the 12 ft × 80 ft limits, drainage outlet, utility markings, truck route, and approved aggregate source.","Document existing elevations and soft areas; stop for owner review if unsuitable subgrade or frost conditions are found.","Mobilize equipment, protect adjacent lawn and structures, and establish a safe delivery and work zone.","Clear and shape the existing path, establish the crown, and prepare the subgrade.","Place the approved Class 5 aggregate in controlled lifts to the selected compacted depth.","Grade, crown, water as needed, compact, and verify drainage and finished width.","Shape edges, remove excess material, take completion photos, and record final quantities and site exceptions."]
    },
    pond: {
      ...
      jobInstructions: ["Confirm final pond location, 10 ft × 14 ft footprint, maximum depth, utility clearances, electrical source, access, and spoil location.","Protect the access path and adjacent lawn, mark the basin and shelves, and establish safe excavation controls.","Excavate and shape the basin; stop for owner review if groundwater, bedrock, utilities, or unstable soils are encountered.","Install and inspect underlayment and EPDM liner before covering any concealed work.","Install pump, filtration, plumbing, and any selected waterfall or lighting components according to manufacturer requirements.","Set natural stone, edge finish, decorative rock, and approved aquatic plants while preventing liner damage.","Fill, leak-test, balance circulation, verify electrical protection, restore disturbed areas, document startup readings, and complete owner walkthrough."]
    },
    clear: {
      ...
      jobInstructions: ["Confirm surveyed limits, approved clearing boundaries, protected trees, wetland and utility markings, access route, debris plan, and permit requirements.","Conduct a site safety briefing, establish exclusion zones, and verify equipment access and emergency procedures.","Clear selected trees and brush in the approved sequence while protecting retained trees, adjacent property, and marked utilities.","Grub stumps only in the approved house and driveway zones unless expanded grubbing is selected.","Consolidate, chip, burn, or haul material only by the approved lawful method; record loads or disposal evidence when applicable.","Rough-grade the house and access zones, control erosion, maintain drainage, and stop for owner review if rock or unsuitable soil is encountered.","Complete cleanup, photograph limits and final grades, record exceptions and quantities, and obtain owner acceptance before demobilization."]
    }
  },
  advertising: {
...
(() => {
  function installJobFlow() {
    ...
    const render = () => {
      ...
      const update = () => { const boxes=[...taskList.querySelectorAll('input')]; boxes.forEach(box=>box.closest('.task-item').classList.toggle('done',box.checked)); status.textContent=`${boxes.filter(box=>box.checked).length} of ${boxes.length} tasks complete`; };
      taskList.querySelectorAll('input').forEach(box=>box.addEventListener('change',update)); update();
    };
    document.querySelectorAll('[data-upgrade]').forEach(box=>box.addEventListener('change',render)); render();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(installJobFlow,0)); else setTimeout(installJobFlow,0);
})();
