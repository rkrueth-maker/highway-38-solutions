...
function boApi(request) {
  ...
  const action = boNormalizeText_(payload.action);
  ...
  const handlers = {
    ...
    voidRecord: function () { return boSoftVoidRecord_(BO_MODULES[args.module] || args.module, args.recordId, args.reason); },
