...
function cbAiQuoteStore_(context,input,draft,providerName,matches){
  var now=cbNow_(),conversationId=cbText_(input.aiConversationId)||cbUuid_('AI-CONVERSATION');
  if(!cbPlatformFindRow_(context.core,'aiConversations','AI Conversation ID',conversationId))cbAppend_(context.core,'aiConversations',{'AI Conversation ID':conversationId,'Business ID':context.row['Business ID'],'User ID':context.user.userId,'Title':cbText_(draft.projectTitle)||'AI quote draft','Context JSON':JSON.stringify({pageKey:'quotes',quoteId:cbText_(input.quoteId),priceBookSearchedFirst:true}),'Status':'Active','Created Time':now,'Updated Time':now,'Record Version':1});
  cbAppend_(context.core,'aiMessages',{'AI Message ID':cbUuid_('AI-MESSAGE'),'Business ID':context.row['Business ID'],'AI Conversation ID':conversationId,'Role':'User','Body':cbAiQuoteText_([input.projectTitle,input.scope,input.measurementNotes,input.notes].join(' ')),'Sources JSON':'[]','Action JSON':'{}','Created Time':now,'Record Version':1});
  cbAppend_(context.core,'aiMessages',{'AI Message ID':cbUuid_('AI-MESSAGE'),'Business ID':context.row['Business ID'],'AI Conversation ID':conversationId,'Role':'Assistant','Body':'Quote draft staged for owner review. Price Book searched first.','Sources JSON':JSON.stringify(matches.map(function(item){return item.row['Item ID'];})),'Action JSON':JSON.stringify(draft),'Created Time':now,'Record Version':1});
...
function cbCompletionAiBuildQuoteDraft_(request){
  var input=request||{},context=cbCompletionContext_(input.businessId,'manageQuotes'),customerId=cbText_(input.customerId),text=cbAiQuoteText_([input.projectTitle,input.scope,input.measurementNotes,input.notes].join(' '));
  cbAssert_(customerId,'Customer selection is required.');cbAssert_(text,'Scope, notes or measurements are required.');
  var matches=cbAiQuotePriceMatches_(context,text),draft={projectTitle:cbText_(input.projectTitle)||'AI-assisted quote draft',scope:cbText_(input.scope),suggestedLines:matches.slice(0,10).map(function(item){return{catalogId:item.row['Item ID'],description:item.row.Description,quantity:1,unit:item.row['Unit of Measure']||'each',rate:Number(item.row['Selling Price']||0),priceStatus:'Owner review required',confidence:'Price Book match',evidence:'Price Book searched first'};}),assumptions:[],exclusions:['No automatic approval, promise or customer send.'],missingInformation:['Confirm critical dimensions, quantities, utilities, access, permits and site conditions.'],photoObservations:[]},provider=cbAiQuoteProvider_(),providerName='Price Book-assisted local fallback';
  if(provider.connected){
    try{
      var priceBook=matches.map(function(item){return{id:item.row['Item ID'],sku:item.row.SKU,description:item.row.Description,category:item.row.Category,unit:item.row['Unit of Measure'],sellingPrice:item.row['Selling Price']};});
      draft=cbAiQuoteResponseJson_(provider,'You are H38 AI inside the Quote Builder. Search the supplied Price Book first. Use only supplied facts. Never invent measurements, quantities, hidden conditions, permits, final pricing or customer promises. Return strict JSON with projectTitle, scope, suggestedLines, assumptions, exclusions, missingInformation and photoObservations. Every suggested line must include catalogId, description, quantity, unit, rate, priceStatus, confidence and evidence. Price status must remain owner review required. Never approve or send.',[{type:'input_text',text:'PROJECT INPUT\n'+text+'\n\nPRICE BOOK MATCHES\n'+JSON.stringify(priceBook)}]);
  ...
  var conversationId=cbAiQuoteStore_(context,input,draft,providerName,matches);
  return{status:'PASS',aiConversationId:conversationId,draft:draft,provider:providerName,priceBookSearchedFirst:true,ownerReviewRequired:true,automaticApproval:false,automaticSend:false,externalActionsEnabled:false};
}
