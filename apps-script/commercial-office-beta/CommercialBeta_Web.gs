...
function cbApi(request){
  ...
  try{
    ...
    if(action==='fullDemoAcceptanceSnapshot')return cbFullDemoAcceptanceSnapshot_(args);
    ...
    if(action==='aiBuildQuoteDraft')return cbCompletionAiBuildQuoteDraft_(args);
    if(action==='aiMeasurePhoto')return cbCompletionAiMeasurePhoto_(args);
    ...
    if(action==='seedFullDemoOffice')return cbCompletionSeedFullDemoOffice_(args);
    if(action==='seedFullDemoBatch')return cbCompletionSeedFullDemoBatch_(args);
    if(action==='seedFullDemoExtras')return cbCompletionSeedFullDemoExtras_(args);
