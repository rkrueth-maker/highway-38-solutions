...
function cbDemo8SeedCoreRecords_(context,project,projectIndex){
  var customerId=cbCompletionEnsureGenericQuoteCustomer_(context),key=project.key,propertyId=cbDemo8Id_('PROPERTY',key),requestId=cbDemo8Id_('REQUEST',key),jobId=cbDemo8Id_('JOB',key),index=Math.max(0,Number(projectIndex||0));
  ...
  return{step:'records',projects:1,quotes:0,cadFiles:0,measurements:0};
}
function cbDemo8SeedCoreQuote_(context,project){
  var customerId=cbCompletionEnsureGenericQuoteCustomer_(context),key=project.key,propertyId=cbDemo8Id_('PROPERTY',key),jobId=cbDemo8Id_('JOB',key),quoteId=cbDemo8Id_('QUOTE',key);
