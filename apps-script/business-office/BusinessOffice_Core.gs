...
function boGetSavedViews(moduleName) {
  ...
  return boReadTable_(H38_BO_SHEETS.SAVED_VIEWS, { includeVoided: true }).filter(function (row) {
...
function boGetNextNumber_(recordType) {
  ...
  try {
    ...
    boUpdateRecord_(H38_BO_SHEETS.NUMBER_SEQUENCES, sequence['Sequence ID'], {
      'Next Number': next + 1,
      'Last Issued': number
    }, 'Number sequence');
    return number;
...
function boAudit_(action, recordType, recordId, previousValues, newValues, source) {
  try {
    ...
    boGetSheet_(H38_BO_SHEETS.AUDIT_LOG).appendRow([
...
function boProof_(action, recordType, recordId, result, evidence, actor) {
  boGetSheet_(H38_BO_SHEETS.PROOF_LOG).appendRow([
...
function boError_(source, recordType, recordId, error, severity) {
  try {
    boGetSheet_(H38_BO_SHEETS.ERROR_LOG).appendRow([
