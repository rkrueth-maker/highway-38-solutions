...
function boUploadDocument(payload) {
  return boSafeExecute_('Document upload', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.DOCUMENTS, 'Create');
    ...
    boAssert_(bytes.length <= H38_BO.MAX_UPLOAD_BYTES, 'File exceeds the 20 MB upload limit.');
...
function boGetDocumentPreview(documentId) {
  const user = boRequirePermission_(H38_BO_SHEETS.DOCUMENTS, 'View');
...
function boVoidDocument(documentId, reason) {
  ...
  const updated = boUpdateRecord_(H38_BO_SHEETS.DOCUMENTS, documentId, {
    'Is Voided': 'Yes',
...
function boExtractDocument(documentId) {
  return boSafeExecute_('OCR-assisted extraction', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.DOCUMENTS, 'Edit');
    const document = boFindRecord_(H38_BO_SHEETS.DOCUMENTS, documentId).record;
    ...
    boUpdateRecord_(H38_BO_SHEETS.DOCUMENTS, documentId, { 'OCR State': 'Processing', 'Upload State': 'Processing' }, 'OCR extraction');
    let text = '';
    try {
    ...
    suggestions.forEach(function (suggestion) {
      boAppendRecord_(H38_BO_SHEETS.OCR_FIELDS, {
...
function boReviewOcrField(ocrFieldId, approvedValue, notes) {
  return boSafeExecute_('OCR field review', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.OCR_FIELDS, 'Edit');
    const field = boFindRecord_(H38_BO_SHEETS.OCR_FIELDS, ocrFieldId, { includeVoided: true }).record;
    ...
    boAppendRecord_(H38_BO_SHEETS.OCR_CORRECTIONS, {
    ...
    const updated = boUpdateRecord_(H38_BO_SHEETS.OCR_FIELDS, ocrFieldId, {
...
function boApproveDocument(documentId) {
  return boSafeExecute_('Approve document', function () {
    const owner = boRequireOwner_();
    const document = boFindRecord_(H38_BO_SHEETS.DOCUMENTS, documentId).record;
    const fields = boReadTable_(H38_BO_SHEETS.OCR_FIELDS, { includeVoided: true }).filter(function (row) { return row['Document ID'] === documentId; });
...
function boPostApprovedDocument(documentId, targetType) {
  return boSafeExecute_('Post approved document', function () {
    ...
    const values = {};
    boReadTable_(H38_BO_SHEETS.OCR_FIELDS, { includeVoided: true }).filter(function (row) {
    ...
    if (targetType === 'Receipt') {
      target = boAppendRecord_(H38_BO_SHEETS.RECEIPTS, {
    ...
    boUpdateRecord_(H38_BO_SHEETS.DOCUMENTS, documentId, { 'Posted Status': 'Posted', 'Upload State': 'Posted' }, 'Approved document posting');
...
function boGeneratePdf(documentType, recordId) {
  return boSafeExecute_('Generate PDF', function () {
    ...
    const template = boReadTable_(H38_BO_SHEETS.PDF_TEMPLATES, { includeVoided: true }).find(function (row) {
    ...
    const folder = DriveApp.getFolderById(boGetFolderId_(H38_BO.PDF_FOLDER_PROPERTY));
...
function boGetPdfSource_(documentType, recordId) {
  const business = boReadTable_(H38_BO_SHEETS.BUSINESSES, { includeVoided: true }).find(function (row) { return row['Business ID'] === boGetBusinessId_(); }) || {};
  const map = {
    Quote: H38_BO_SHEETS.QUOTES,
    Estimate: H38_BO_SHEETS.QUOTES,
    'Work Order': H38_BO_SHEETS.WORK_ORDERS,
    'Purchase Order': H38_BO_SHEETS.PURCHASE_ORDERS,
    Invoice: H38_BO_SHEETS.INVOICES,
    'Payment Receipt': H38_BO_SHEETS.PAYMENTS,
    'Expense Report': H38_BO_SHEETS.EXPENSES,
    'Job Cost Report': H38_BO_SHEETS.JOBS,
    'Customer Project Summary': H38_BO_SHEETS.JOBS,
    'Payroll Summary': H38_BO_SHEETS.PAYROLL_PERIODS,
    'Sales Tax Summary': H38_BO_SHEETS.TAX_PERIODS,
    'Tax Preparation Packet': H38_BO_SHEETS.TAX_PERIODS,
    'Year-End Accountant Package': H38_BO_SHEETS.TAX_PERIODS
  };
