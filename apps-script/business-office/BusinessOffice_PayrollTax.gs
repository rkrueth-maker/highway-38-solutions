...
function boPreparePayrollPeriod(payload) {
  return boSafeExecute_('Prepare payroll period', function () {
    ...
    const period = boAppendRecord_(H38_BO_SHEETS.PAYROLL_PERIODS, {
    ...
    const timeRows = boReadTable_(H38_BO_SHEETS.TIME_ENTRIES).filter(function (row) {
      return row['Approval Status'] === 'Approved' && row.Date >= payload.periodStart && row.Date <= payload.periodEnd;
    });
    const employees = boReadTable_(H38_BO_SHEETS.EMPLOYEES).filter(function (row) { return row.Status === 'Active'; });
    employees.forEach(function (employee) {
      ...
      const deductions = boReadTable_(H38_BO_SHEETS.PAYROLL_DEDUCTIONS).filter(function (row) {
      ...
      boAppendRecord_(H38_BO_SHEETS.PAYROLL_LINES, {
    ...
    return boFindRecord_(H38_BO_SHEETS.PAYROLL_PERIODS, periodId).record;
...
function boExportPayrollProviderCsv(periodId) {
  return boSafeExecute_('Payroll provider export', function () {
    ...
    const period = boFindRecord_(H38_BO_SHEETS.PAYROLL_PERIODS, periodId).record;
    ...
    boAssert_(!H38_BO.DIRECT_PAYROLL_FUNDING, 'This system does not move payroll funds.');
    const lines = boReadTable_(H38_BO_SHEETS.PAYROLL_LINES).filter(function (row) { return row['Payroll Period ID'] === periodId; });
    ...
    const folder = DriveApp.getFolderById(boGetFolderId_(H38_BO.EXPORT_FOLDER_PROPERTY));
    const file = folder.createFile('payroll-provider-export-' + periodId + '.csv', csv, MimeType.CSV);
    boUpdateRecord_(H38_BO_SHEETS.PAYROLL_PERIODS, periodId, { 'Export File ID': file.getId(), Status: 'Exported' }, 'Payroll provider export');
    boAppendRecord_(H38_BO_SHEETS.EXPORT_JOBS, {
...
function boPrepareSalesTaxPeriod(payload) {
  return boSafeExecute_('Prepare sales tax period', function () {
    ...
    const invoices = boReadTable_(H38_BO_SHEETS.INVOICES).filter(function (invoice) {
    ...
    const period = boAppendRecord_(H38_BO_SHEETS.TAX_PERIODS, {
...
function boFinalizeTaxPreparationReport(periodId) {
  return boSafeExecute_('Finalize tax preparation report', function () {
    const user = boRequireRestrictedArea_('tax');
    boRequireRestrictedArea_('export');
    const period = boFindRecord_(H38_BO_SHEETS.TAX_PERIODS, periodId).record;
    ...
    boAssert_(!H38_BO.DIRECT_TAX_FILING, 'This system does not file tax returns.');
    const pdf = boGeneratePdf('Tax Preparation Packet', periodId);
    boUpdateRecord_(H38_BO_SHEETS.TAX_PERIODS, periodId, { Status: 'Finalized', 'Report File ID': pdf.fileId }, 'Tax report finalization');
...
function boGetTaxPreparationSummary() {
  ...
  return {
    taxPrep: boReadTable_(H38_BO_SHEETS.TAX_PREP, { allBusinesses: true, includeVoided: true }),
    salesTax: boReadTable_(H38_BO_SHEETS.SALES_TAX_REPORT, { allBusinesses: true, includeVoided: true }),
    contractors: boReadTable_(H38_BO_SHEETS.CONTRACTORS),
    missingDocuments: boReadTable_(H38_BO_SHEETS.MISSING_DOCUMENTS),
    assets: boReadTable_(H38_BO_SHEETS.ASSETS),
    mileage: boReadTable_(H38_BO_SHEETS.MILEAGE),
    homeOffice: boReadTable_(H38_BO_SHEETS.HOME_OFFICE),
    boundary: H38_BO.TAX_BOUNDARY
  };
}
