...
function boCreateCustomerFromRequest(requestId) {
  return boSafeExecute_('Create customer from request', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.CUSTOMERS, 'Create');
    const request = boFindRecord_(H38_BO_SHEETS.REQUESTS, requestId).record;
    const duplicateKey = boGetBusinessId_() + '|' + boNormalizeText_(request.Email).toLowerCase();
    const existing = boReadTable_(H38_BO_SHEETS.CUSTOMERS, { includeVoided: true }).find(function (row) {
      return boNormalizeText_(row.Email).toLowerCase() === boNormalizeText_(request.Email).toLowerCase() && row.Status !== 'Voided';
    });
    if (existing) return existing;
    const contact = boAppendRecord_(H38_BO_SHEETS.CONTACTS, {
    ...
    const customer = boAppendRecord_(H38_BO_SHEETS.CUSTOMERS, {
    ...
    boUpdateRecord_(H38_BO_SHEETS.REQUESTS, requestId, {
...
function boCreateQuote(payload) {
  return boSafeExecute_('Create quote', function () {
    ...
    const customer = boFindRecord_(H38_BO_SHEETS.CUSTOMERS, payload.customerId).record;
    const quoteId = boId_('QUOTE');
    const quoteNumber = boGetNextNumber_('Quote');
    const quote = boAppendRecord_(H38_BO_SHEETS.QUOTES, {
      'Quote ID': quoteId,
    ...
    payload.lines.forEach(function (line, index) {
      ...
      boAppendRecord_(H38_BO_SHEETS.QUOTE_LINES, {
    ...
    return boFindRecord_(H38_BO_SHEETS.QUOTES, quoteId).record;
  }, 'Quote', payload && payload.quoteId);
}

function boReviseQuote(quoteId, changes) {
  return boSafeExecute_('Revise quote', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.QUOTES, 'Edit');
    const original = boFindRecord_(H38_BO_SHEETS.QUOTES, quoteId).record;
    ...
    boUpdateRecord_(H38_BO_SHEETS.QUOTES, quoteId, { 'Revision Status': 'Superseded', Status: 'Revised' }, 'Quote revision');
    ...
    boAppendRecord_(H38_BO_SHEETS.QUOTES, newQuote, 'Quote revision');
    boReadTable_(H38_BO_SHEETS.QUOTE_LINES).filter(function (line) { return line['Quote ID'] === quoteId; }).forEach(function (line) {
      ...
      boAppendRecord_(H38_BO_SHEETS.QUOTE_LINES, copy, 'Quote revision');
    });
...
function boApproveSelectedRecord(recordType, recordId, approvalType, decision, notes) {
  return boSafeExecute_('Owner approval', function () {
    ...
    const approval = boReadTable_(H38_BO_SHEETS.APPROVALS, { includeVoided: true }).find(function (row) {
    ...
    if (approval) boUpdateRecord_(H38_BO_SHEETS.APPROVALS, approval['Approval ID'], values, 'Owner approval');
    else boAppendRecord_(H38_BO_SHEETS.APPROVALS, Object.assign({ 'Approval ID': boId_('APP') }, values), 'Owner approval');
...
function boApplyApprovalToRecord_(recordType, recordId, approvalType, decision) {
  ...
  const map = {
    Quote: { sheet: H38_BO_SHEETS.QUOTES, patch: { 'Approval Status': decision, 'Send Allowed': approved ? 'Yes' : 'No' } },
    Invoice: { sheet: H38_BO_SHEETS.INVOICES, patch: { 'Approval Status': decision, 'Send Allowed': approved ? 'Yes' : 'No' } },
    'Work Order': { sheet: H38_BO_SHEETS.WORK_ORDERS, patch: { 'Approval Status': decision } },
    'Purchase Order': { sheet: H38_BO_SHEETS.PURCHASE_ORDERS, patch: { 'Approval Status': decision } },
    Expense: { sheet: H38_BO_SHEETS.EXPENSES, patch: { 'Approval Status': decision } },
    'Payroll Period': { sheet: H38_BO_SHEETS.PAYROLL_PERIODS, patch: { 'Approval Status': decision, 'Export Allowed': approved ? 'Yes' : 'No' } },
    'Tax Period': { sheet: H38_BO_SHEETS.TAX_PERIODS, patch: { 'Approval Status': decision, 'Finalization Allowed': approved ? 'Yes' : 'No' } },
    'Journal Entry': { sheet: H38_BO_SHEETS.JOURNAL_ENTRIES, patch: { 'Approval Status': decision, 'Posting Allowed': approved ? 'Yes' : 'No' } }
  };
...
function boConvertQuoteToWorkOrderAndJob(quoteId) {
  return boSafeExecute_('Quote conversion', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.JOBS, 'Create');
    const quote = boFindRecord_(H38_BO_SHEETS.QUOTES, quoteId).record;
    boAssert_(quote['Approval Status'] === 'Approved', 'The quote must be approved before conversion.');
    const existingJob = boReadTable_(H38_BO_SHEETS.JOBS, { includeVoided: true }).find(function (row) {
      return row['Quote ID'] === quoteId && row.Status !== 'Voided';
    });
    if (existingJob) return { job: existingJob, workOrder: boFindRecord_(H38_BO_SHEETS.WORK_ORDERS, existingJob['Work Order ID']).record, duplicatePrevented: true };
    ...
    const workOrder = boAppendRecord_(H38_BO_SHEETS.WORK_ORDERS, {
    ...
    const job = boAppendRecord_(H38_BO_SHEETS.JOBS, {
      'Job ID': jobId,
    ...
    boUpdateRecord_(H38_BO_SHEETS.QUOTES, quoteId, { 'Job ID': jobId, Status: 'Converted' }, 'Quote conversion');
...
function boCreateInvoiceFromJob(jobId) {
  return boSafeExecute_('Job invoice creation', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.INVOICES, 'Create');
    const job = boFindRecord_(H38_BO_SHEETS.JOBS, jobId).record;
    const existing = boReadTable_(H38_BO_SHEETS.INVOICES, { includeVoided: true }).find(function (row) {
      return row['Job ID'] === jobId && row.Status !== 'Voided';
    });
    if (existing) return existing;
    const invoiceId = boId_('INV');
    const invoiceNumber = boGetNextNumber_('Invoice');
    const invoice = boAppendRecord_(H38_BO_SHEETS.INVOICES, {
    ...
    const quoteLines = boReadTable_(H38_BO_SHEETS.QUOTE_LINES).filter(function (line) { return line['Quote ID'] === job['Quote ID']; });
    quoteLines.forEach(function (line, index) {
      boAppendRecord_(H38_BO_SHEETS.INVOICE_LINES, {
    ...
    boUpdateRecord_(H38_BO_SHEETS.JOBS, jobId, { 'Invoice Status': 'Draft Invoice Created' }, 'Job invoice creation');
    boProof_('JOB TO INVOICE', 'Job', jobId, 'PASS', invoiceNumber, user.Email);
    return boFindRecord_(H38_BO_SHEETS.INVOICES, invoiceId).record;
  }, 'Job', jobId);
}

function boMatchVendorBillToPurchaseOrder(billId, purchaseOrderId) {
  return boSafeExecute_('Vendor bill matching', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.VENDOR_BILLS, 'Edit');
    const bill = boFindRecord_(H38_BO_SHEETS.VENDOR_BILLS, billId).record;
    const po = boFindRecord_(H38_BO_SHEETS.PURCHASE_ORDERS, purchaseOrderId).record;
    ...
    boUpdateRecord_(H38_BO_SHEETS.VENDOR_BILLS, billId, { 'PO ID': purchaseOrderId, Status: 'Matched' }, 'Vendor bill matching');
    boUpdateRecord_(H38_BO_SHEETS.PURCHASE_ORDERS, purchaseOrderId, { 'Vendor Bill Status': 'Matched' }, 'Vendor bill matching');
...
function boConvertReceiptToExpense(receiptId) {
  return boSafeExecute_('Receipt to expense', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.EXPENSES, 'Create');
    const receipt = boFindRecord_(H38_BO_SHEETS.RECEIPTS, receiptId).record;
    boAssert_(receipt['Approval Status'] === 'Approved', 'Receipt review and approval are required before expense creation.');
    const existing = boReadTable_(H38_BO_SHEETS.EXPENSES, { includeVoided: true }).find(function (row) {
      return row['Receipt ID'] === receiptId && row.Status !== 'Voided';
    });
    if (existing) return existing;
    const expense = boAppendRecord_(H38_BO_SHEETS.EXPENSES, {
...
function boRecordPayment(payload) {
  return boSafeExecute_('Record payment', function () {
    const user = boRequirePermission_(H38_BO_SHEETS.PAYMENTS, 'Create');
    const invoice = boFindRecord_(H38_BO_SHEETS.INVOICES, payload.invoiceId).record;
    ...
    const payment = boAppendRecord_(H38_BO_SHEETS.PAYMENTS, {
...
function boPrepareCustomerAction(recordType, recordId) {
  const map = {
    Quote: { sheet: H38_BO_SHEETS.QUOTES, allowed: 'Send Allowed' },
    Invoice: { sheet: H38_BO_SHEETS.INVOICES, allowed: 'Send Allowed' },
    'Payment Receipt': { sheet: H38_BO_SHEETS.PAYMENTS, allowed: 'Delivery Allowed' }
  };
  const target = map[recordType];
  ...
  boAssert_(!H38_BO.EXTERNAL_ACTIONS_ENABLED, 'External actions remain locked in this build.');
