/** Highway 38 Business Office — non-destructive self-test suite. */
...
function boRunSelfTest() {
  ...
  test('Authorized user and role', function () {
    ...
    boAssert_(role && H38_BO.ROLES.indexOf(role['Role Name']) >= 0, 'Invalid role.');
  ...
  test('External action boundaries', function () {
    boAssert_(H38_BO.EXTERNAL_ACTIONS_ENABLED === false, 'External actions must remain disabled.');
    boAssert_(H38_BO.DIRECT_PAYMENT_PROCESSING === false, 'Direct payment processing must remain disabled.');
    boAssert_(H38_BO.DIRECT_PAYROLL_FUNDING === false, 'Direct payroll funding must remain disabled.');
    boAssert_(H38_BO.DIRECT_TAX_FILING === false, 'Direct tax filing must remain disabled.');
  ...
  test('Proof and error logs', function () {
    const proofCount = boReadTable_(H38_BO_SHEETS.PROOF_LOG, { includeVoided: true }).length;
  ...
  const result = { status: failed.length ? 'HOLD' : 'PASS', tests: tests, testedAt: boNow_(), testedBy: user.Email, version: H38_BO.VERSION };
