'use strict';
const H38_STARTUP_BUILD='20260804-nlpm-trial-1';
const H38_LOCAL_STARTUP_TIMEOUT=3000;
const NLPM_SELECTED_BUSINESS_KEY='nlpm-selected-business';
...
function setFastBusinessId(businessId){
  ...
  try{if(state.businessId)localStorage.setItem(NLPM_SELECTED_BUSINESS_KEY,state.businessId);else localStorage.removeItem(NLPM_SELECTED_BUSINESS_KEY);}catch(error){}
}
...
function armStartupWatchdog(){clearTimeout(h38StartupWatchdog);h38StartupWatchdog=setTimeout(()=>{if(!state.snapshot)renderWelcome('unavailable');},12000);}
async function retireLegacyOfflineShell(){
  try{
    if('serviceWorker' in navigator){const registrations=await navigator.serviceWorker.getRegistrations();await Promise.all(registrations.filter(registration=>registration.scope.includes('/businesses/northern-lakes/commercial-app/')).map(registration=>registration.unregister()));}
    if('caches' in window){const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith('nlpm-business-office-')).map(key=>caches.delete(key)));}
  }catch(error){console.warn(error.message||String(error));}
}
function secureOfficeUrl(){return `/businesses/northern-lakes/business-office.html?officeBuild=${encodeURIComponent(H38_STARTUP_BUILD)}`;}
...
function saveStartupSnapshot(snapshot,businessId){
  if(!snapshot||!snapshot.business||!snapshot.user)throw new Error('Secure startup returned an incomplete Northern Lakes business pack.');
...
async function hydrateLocalStartup(requestedBusinessId){
  ...
  else{try{const selected=await withStartupTimeout(get('meta','selectedBusiness'),'loading saved business');const cachedBusinessId=selected?.businessId||'';if(cachedBusinessId&&!state.businessId)setFastBusinessId(cachedBusinessId);}catch(error){console.warn(error.message);}}
  try{
...
async function init(){
  ...
  const query=new URLSearchParams(location.search);state.shell=SHELL_PAGES[query.get('shell')]?query.get('shell'):'office';$('shellLabel').textContent=state.shell==='quote'?'Northern Lakes Quote Builder':'Northern Lakes Business Office';
  const requestedBusinessId=(query.get('businessId')||'').trim();let fastBusinessId=requestedBusinessId;
  if(!fastBusinessId){try{fastBusinessId=localStorage.getItem(NLPM_SELECTED_BUSINESS_KEY)||'';}catch(error){}}
...
function handleBridgeStatus(status){
  ...
  if(status==='connected'){$('businessStatus').textContent='Secure Northern Lakes gateway connected · opening Office…';return;}
  if(status==='bootstrapped'){clearTimeout(h38StartupWatchdog);return;}
  if(status==='auth-expired'){state.bridgeReady=false;$('businessStatus').textContent=state.snapshot?'Office open offline · secure session expired.':'Secure session expired.';if(!state.snapshot)renderWelcome('unavailable','Sign in again to reopen the secure Northern Lakes Office.');else toast('Secure session expired. Your work remains saved on this device.',true);return;}
  if(status==='sign-in-required'){state.bridgeReady=false;$('businessStatus').textContent=state.snapshot?'Offline Northern Lakes business pack · sign in to refresh.':'Sign in required.';if(!state.snapshot)renderWelcome('unavailable');}
}
async function handleStartupBootstrap(startup){
  try{
    const businesses=Array.isArray(startup?.businesses)?startup.businesses:[];if(!businesses.length)throw new Error('No active Northern Lakes business is assigned to this account.');
    ...
    if(startup.snapshot){saveStartupSnapshot(startup.snapshot,state.businessId);$('businessStatus').textContent=`${startup.snapshot.business.businessName} · ${startup.snapshot.user.roleName} · Office open · refreshing latest records…`;openPage(state.page,false);updatePending().catch(()=>{});state.bridge.request('fullStartupRefresh',{businessId:state.businessId},120000).then(snapshot=>handleFullSnapshot(snapshot,state.businessId)).catch(error=>handleBridgeError('refresh',error.message||String(error)));return;}
    if(state.canSwitchBusinesses){$('businessStatus').textContent='Choose a Northern Lakes business to open.';renderWelcome('choose');return;}
    throw new Error('Your assigned Northern Lakes business could not be selected.');
  }catch(error){handleBridgeError('startup',error.message||String(error));}
}
async function handleFullSnapshot(snapshot,businessId){
  try{const id=businessId||snapshot?.business?.businessId;if(!id||id!==state.businessId)return;saveStartupSnapshot(snapshot,id);$('businessStatus').textContent=`${snapshot.business.businessName} · ${snapshot.user.roleName} · latest records loaded ${new Date().toLocaleTimeString()}`;openPage(state.page,false);updatePending().catch(()=>{});sync(false).catch(()=>{});toast('Latest Northern Lakes records loaded.');}
...
function handleBridgeError(stage,message){
  ...
  state.bridgeReady=false;$('businessStatus').textContent='Northern Lakes Business Office connection failed.';toast(text,true);if(!state.snapshot)renderWelcome('error',text);
}
function populateBusinessSelector(businesses){const select=$('businessSelect');select.innerHTML='<option value="">Select business</option>'+businesses.map(b=>`<option value="${esc(b.businessId)}">${esc(b.businessName)}${industryPacks(b).length?' — '+esc(industryPacks(b).join(', ')):''}</option>`).join('');select.value=state.businessId||'';}
function bindGlobal(){
...
function renderWelcome(mode='connecting',detailOverride=''){
  ...
  const title=unavailable?'Open Northern Lakes Business Office securely':choose?'Choose a business':'Opening Northern Lakes Business Office…';
  const detail=detailOverride||(unavailable?'Sign in with Google in this tab. The secure gateway returns here; no second window stays open.':choose?'Use the owner-only business selector above.':'Reading the secure Northern Lakes gateway handoff.');
