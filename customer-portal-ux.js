(function(){
  ...
  function openQuoteReview(quoteId){
    ...
    details.innerHTML=[
      ['Deliverables',quote.deliverables||'Highway 38 must post the complete deliverable list before approval.'],
      ['Price',money(quote.amount)],
      ['Timing',quote.timing||'Timing has not been posted.'],
      ['Revision allowance',quote.revision_allowance||'Revision allowance has not been posted.'],
      ['Exclusions',quote.exclusions||'Exclusions have not been posted.'],
      ['Approval consequence',quote.approval_consequence||'Approval records your decision for Highway 38 owner review. It does not charge a card, send a message, or begin work automatically.']
  ...
  function nav(){
    ...
    links.forEach(link=>link.addEventListener('click',()=>{
      ...
      byId('portalMoreMenu').hidden=true;
      byId('portalMoreButton')?.setAttribute('aria-expanded','false');
    }));
    const more=byId('portalMoreButton'),menu=byId('portalMoreMenu');
    more?.addEventListener('click',()=>{const open=menu.hidden;menu.hidden=!open;more.setAttribute('aria-expanded',String(open));if(open){menu.style.position='fixed';menu.style.right='.75rem';menu.style.bottom='74px';menu.style.zIndex='70';menu.style.display='grid';menu.style.gap='.5rem';}else menu.style.display='';});
  }
  function boot(){
    ...
    byId('closeQuoteReview')?.addEventListener('click',()=>byId('quoteReviewDialog')?.close());
    ...
    byId('quoteApproveConfirmed')?.addEventListener('click',()=>{const quote=quoteById(activeQuoteId);if(!quote)return;window.H38_CUSTOMER_PORTAL?.approveQuote(quote.id,Number(byId('quoteApproveConfirmed').dataset.version||quote.version||1));byId('quoteReviewDialog')?.close();});
