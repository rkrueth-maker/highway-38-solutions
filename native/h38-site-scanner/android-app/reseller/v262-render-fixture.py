...
out=Path(sys.argv[2]) if len(sys.argv)>2 else Path('/tmp/h38-v262-render')
...
assert needle in s
stub=r'''<script>
window.__H38_V262_RENDER_FIXTURE=true;
...
const __clickTimer=setInterval(()=>{
  try{
    ...
    if(!__clicked&&window.H38_SCOUT_V262_PHONE_VIDEO_REPAIR_INSTALLED===true&&authorized&&b&&/Search public Facebook/.test(b.textContent||'')){
