...
// Penny truth remains owned by v200-hunt.js. This layer only repairs lifecycle, ranking and recovery around it.
...
loadInventory=async function(){
  ...
  const map=new Map();
  for(const x of remote)map.set(h38InventoryKey(x),x);
  for(const x of locals){const k=h38InventoryKey(x);if(!map.has(k))map.set(k,x)}
...
recordPurchase=async function(r){
  ...
  try{
    const{data,error:e}=await h38sb.from('reseller_deals').insert(payload).select('*').single();if(e)throw e;
    saveLocalInventory(localInventory().filter(x=>h38InventoryKey(x)!==key));
    await loadInventory();notice(`${payload.title} saved to shared Inventory.`,'good');return data;
...
async function h38SaveSaleOutcome(id){
  const x=(state.inventory||[]).find(r=>txt(r.id)===txt(id));if(!x)return;
  const sold=num($('salePrice')?.value),fees=num($('saleFees')?.value);if(!(sold>0)){notice('Enter the actual sold price.','warn');return}
...
// Exact-UPС image recovery: strict penny retailers may enter the server resolver with barcode proof even when no image URL exists.
...
// One more maintenance layer verifies the repaired shared lifecycle and the full penny/image/watch contract.
const h38v211Maintenance=runMaintenance;
runMaintenance=async function(){if(state.maintenance.running)return;await h38v211Maintenance();state.maintenance.running=true;state.maintenance.tests=state.maintenance.tests.filter(x=>x.name!=='Overall');renderMaintenance();try{
  ...
  try{const p=await fn('reseller-image-delivery-v201',{items:[]},30000);addTest('Image delivery service',p?.status==='PASS'?'pass':'fail','Exact-UPС recovery service is authenticated and reachable; missing images do not change penny truth.')}catch(e){addTest('Image delivery service','fail',txt(e.message||e))}
  try{const old=watchRows(),term='H38 ranking fixture';addWatch(term,{min_profit:40,min_roi:30,max_buy:80});const pass=h38WatchEvaluation({title:term,term,buy_price:50,net_profit:60,roi_pct:120}),fail=h38WatchEvaluation({title:term,term,buy_price:90,net_profit:20,roi_pct:22});removeWatch(term);write(H38_KEYS.watch,old);addTest('Watch ranking logic',pass?.status==='hit'&&fail?.status==='over'?'pass':'fail','Profit/ROI/max-buy targets now influence deal priority and badges.')}catch(e){addTest('Watch ranking logic','fail',txt(e.message||e))}
