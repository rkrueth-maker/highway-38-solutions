...
(function(){
  ...
  function agePass(l,f){const a=age(l),elapsed=Date.now()-a.ms;switch(f){case'48h':return a.known&&elapsed<=2*86400000;case'7d':return a.known&&elapsed<=7*86400000;case'8-14':return a.known&&a.days>=8&&a.days<=14;case'15-30':return a.known&&a.days>=15&&a.days<=30;case'31+':return a.known&&a.days>=31;case'unknown':return!a.known;default:return true}}
  ...
  function enhanceClearance(){
    ...
    if(state.v053GroupMode==='store'){
      ...
      if($('v053Store'))$('v053Store').value=selected.store_key;
  ...
  renderClearance=function(){const useSnapshot=state.v053Refreshing&&(!Array.isArray(state.leads)||!state.leads.length)&&Array.isArray(state.v053RefreshSnapshot)&&state.v053RefreshSnapshot.length;if(useSnapshot){const live=state.leads;state.leads=state.v053RefreshSnapshot;try{renderClearanceV053Base();enhanceClearance()}finally{state.leads=live}}else{renderClearanceV053Base();enhanceClearance()}};
