...
window.H38_V056_RUNTIME_MARKER='stores-resale-sources-v056-v057-reliability-fix';
window.H38_V057_STORE_FIX_ACTIVE=true;
(function(){
  ...
  state.v056SourceLoading=!!state.v056SourceLoading;

  function addStyle(){
    if(document.getElementById('h38-v056-style'))return;
    const s=document.createElement('style');s.id='h38-v056-style';s.textContent=`
      .v056-store-tabs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin-top:8px}.v056-store-tabs button{min-height:42px}.v056-source-head{display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:8px}.v056-source-grid{display:grid;gap:8px}.v056-source-card{padding:10px 11px}.v056-source-top{display:flex;justify-content:space-between;gap:9px;align-items:flex-start}.v056-source-top strong{display:block}.v056-source-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:9px}.v056-source-actions button{width:auto}.v056-source-filter{min-height:42px;width:100%;margin-top:8px}.v056-truth{margin-top:8px}.v056-hidden{display:none!important}.v057-source-progress{margin-top:7px}.v057-other-row{display:flex;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid #edf1f4}.v057-other-row:last-child{border-bottom:0}@media(max-width:520px){.v056-store-tabs{grid-template-columns:1fr}.v056-source-top{display:block}.v056-source-top>strong{margin-top:6px}}
    `;document.head.appendChild(s);
  }
  addStyle();

  async function loadResaleSources(force){
    if(!hasPoint()||state.v056SourceLoading)return;
    state.v056SourceLoading=true;
    state.v056SourceStatus=force?'Refreshing closest resale-source map regions…':'Checking closest resale-source map regions…';
    if(state.page==='stores')renderStores();
    try{
      const p=await fn('reseller-nearby-sources',{lat:Number(state.location.lat),lon:Number(state.location.lon),radiusMiles:radius()},30000);
      const incoming=Array.isArray(p.sources)?p.sources:[];
      if(incoming.length||p.scan_complete)state.resaleSources=incoming;
      const coverage=Number(p.coverage_tiles_total)>0?`${Number(p.coverage_tiles_done||0)}/${Number(p.coverage_tiles_total)} map regions`:'map coverage pending';
      state.v056SourceStatus=p.warning?`${coverage} · ${String(p.warning)}`:`${coverage} checked${p.scan_complete?' · complete':''}`;
      diag('resaleSources',{status:p.status||'PASS',count:state.resaleSources.length,coverage_tiles_done:p.coverage_tiles_done||0,coverage_tiles_total:p.coverage_tiles_total||0,batch_successes:p.batch_successes||0,provider_errors:p.provider_errors||[],warning:p.warning||''});
    }catch(e){
      state.v056SourceStatus='Resale-source lookup unavailable; previously saved places were kept. '+String(e?.message||e||'unknown error');
      err('resaleSources',e);
    }finally{
      state.v056SourceLoading=false;
      if(state.page==='stores')renderStores();
    }
  }
  ...
  loadStores=async function(force){
    ...
    if(hasPoint())void loadResaleSources(force);
    return result;
  };
  ...
  function renderSourceSection(p){
    ...
    const cats=[...new Set((state.resaleSources||[]).map(s=>text(s.source_category)).filter(Boolean))].sort(),rows=sourceRows(),section=document.createElement('section');section.className='v056-source-section';
    section.innerHTML=`<div class="card"><div class="v056-source-head"><div><h3>Nearby resale sources</h3><div class="small muted">Antique/vintage, Habitat ReStore, thrift/charity, consignment, pawn, flea, salvage/reuse, liquidation/surplus and other secondhand sourcing places found from map data.</div></div><span class="badge">${rows.length}</span></div><div class="actions"><button id="v057RefreshSources" class="secondary" ${state.v056SourceLoading?'disabled':''}>${state.v056SourceLoading?'Checking…':'Refresh sources'}</button></div><select id="v056SourceCategory" class="v056-source-filter"><option value="all">All source types</option>${cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select>${state.v056SourceStatus?`<div class="tiny muted v057-source-progress">${esc(state.v056SourceStatus)}</div>`:''}</div><div class="v056-source-grid">${rows.length?rows.map(sourceCard).join(''):'<div class="card empty">No resale-source places are loaded yet. Refresh sources to check the closest unfinished map regions; saved places stay visible during retries.</div>'}</div>`;
    ...
    const refresh=$('v057RefreshSources');if(refresh)refresh.onclick=()=>void loadResaleSources(true);
  ...
  function renderOtherNearby(p,groups){
    const old=p.querySelector('.v057-other-nearby');if(old)old.remove();if(state.v056StoreView!=='all')return;
    const zeroKeys=new Set(groups.filter(g=>/\b0 actionable chain-wide lead/i.test(text(g.querySelector('summary')?.textContent))).map(g=>text(g.dataset.v049Retailer))),rows=(state.stores||[]).filter(s=>zeroKeys.has(norm(s.retailer))).sort((a,b)=>n(a.distance_miles)-n(b.distance_miles)),section=document.createElement('section');section.className='card v057-other-nearby';
    const shown=rows.slice(0,20);section.innerHTML=`<details><summary><strong>Other nearby retailers</strong><div class="small muted">${rows.length} zero-deal location${rows.length===1?'':'s'} kept out of the main hunt cards.</div></summary><div style="margin-top:8px">${shown.map(s=>`<div class="v057-other-row"><div><strong>${esc(s.store_name||s.retailer||'Store')}</strong><div class="tiny muted">${esc(s.store_address||s.retailer||'')}</div></div><span class="small">${Number.isFinite(Number(s.distance_miles))?Number(s.distance_miles).toFixed(1)+' mi':'—'}</span></div>`).join('')}${rows.length>shown.length?`<div class="tiny muted" style="margin-top:7px">+${rows.length-shown.length} more zero-deal locations not expanded here.</div>`:''}</div></details>`;p.appendChild(section);
  }

  const renderStoresV056Base=renderStores;
  renderStores=function(){
    state.v053StoreVisibility='leads';
    ...
    tabs.querySelectorAll('[data-v056-view]').forEach(b=>{b.classList.toggle('active',b.dataset.v056View===state.v056StoreView);b.onclick=()=>{state.v056StoreView=b.dataset.v056View;renderStores();if(b.dataset.v056View!=='deals'&&!state.resaleSources.length&&!state.v056SourceLoading)void loadResaleSources(false)}});
