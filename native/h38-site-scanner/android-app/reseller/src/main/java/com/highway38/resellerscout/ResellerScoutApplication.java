...
public final class ResellerScoutApplication extends Application implements Application.ActivityLifecycleCallbacks {
    ...
    private static final String OPPORTUNITY_JS = """
            (function(){
              'use strict';
              if(window.__H38OpportunityEngineV1){if(window.H38OpportunityRefresh)window.H38OpportunityRefresh();return;}
              ...
              function firstUrl(text){var s=String(text||''),i=s.indexOf('http://');if(i<0)i=s.indexOf('https://');if(i<0)return '';var tail=s.slice(i),space=tail.search(/\\s/);return space<0?tail:tail.slice(0,space)}
