...
final class FacebookMarketplaceEmbeddedCollector {
    ...
    private void createCollectorWebView(){
        collector=new WebView(activity);collector.setBackgroundColor(android.graphics.Color.TRANSPARENT);collector.setAlpha(0.01f);collector.setFocusable(false);collector.setFocusableInTouchMode(false);collector.setClickable(false);collector.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO_HIDE_DESCENDANTS);collector.setTranslationX(-5000f);collector.setTranslationY(-5000f);
    ...
    private static String stateAbbrev(String s){String x=s==null?"":s.trim().toLowerCase(Locale.US);Map<String,String> m=Map.ofEntries(Map.entry("minnesota","mn"),Map.entry("ohio","oh"),Map.entry("wisconsin","wi"),Map.entry("michigan","mi"),Map.entry("iowa","ia"),Map.entry("north dakota","nd"),Map.entry("south dakota","sd"));return m.getOrDefault(x,x.length()==2?x:"");}
    private boolean locationMatches(String value){
        String page=value==null?"":value.trim();if(page.isBlank()||postal.isBlank())return false;
    ...
    private void finishPass(){currentTerm="";generation++;try{activity.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().putString(LAST_STATUS,authBlocked?"AUTH_REQUIRED":"COMPLETE").apply();}catch(Exception ignored){}stopCollector(false);scoutWebView.postDelayed(()->scoutWebView.evaluateJavascript("window.H38ScoutReturned&&window.H38ScoutReturned();",null),120);}
