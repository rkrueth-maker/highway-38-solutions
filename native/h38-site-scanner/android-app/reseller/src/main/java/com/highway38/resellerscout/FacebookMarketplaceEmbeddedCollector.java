...
final class FacebookMarketplaceEmbeddedCollector {
    ...
    private void configureWebView(WebView w,boolean visible){
        ...
        // Hidden collector stays behind Scout with a real viewport so Marketplace lazy-loading can render.
        w.setAlpha(1f);
        ...
        // Use the normal Android WebView user agent. Appending a scraper token makes Facebook's session path more brittle.
    ...
    private void createCollectorWebView(){
        ...
        // Index 0 keeps Facebook visually behind the Scout WebView without moving it outside the viewport.
            ...
            @Override public void onPageFinished(WebView view,String url){
                ...
                if(hasFacebookUserCookie()&&!low.contains("/login")&&!low.contains("/checkpoint")){
                    try{
                        ...
                        activity.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().putString(LAST_STATUS,"CONNECTED").apply();
                    }catch(Exception ignored){}
    ...
    private boolean hasFacebookUserCookie(){
        try{
            ...
            return c!=null&&c.contains("c_user=");
        }catch(Exception ignored){
            return false;
        }
    }
    ...
    private void closeConnect(boolean connected){
        FrameLayout layer=connectLayer;
        connectLayer=null;
        if(layer!=null){
            try{
        ...
        if(connected){
            scoutWebView.postDelayed(
                ()->scoutWebView.evaluateJavascript("window.H38FacebookConnected&&window.H38FacebookConnected();",null),
                150
            );
        }
    }

    private int coarseRadius(){
        if(radiusMiles<=25)return 40;
        if(radiusMiles<=50)return 60;
        if(radiusMiles<=100)return 100;
        return 250;
    }
    ...
    private static String norm(String v){
        return v==null?"":v.toLowerCase(Locale.US).replaceAll("[^a-z0-9]","");
    }

    private static String stateAbbrev(String s){
        String x=s==null?"":s.trim().toLowerCase(Locale.US);
        if("minnesota".equals(x))return"mn";
        if("ohio".equals(x))return"oh";
        if("wisconsin".equals(x))return"wi";
        if("michigan".equals(x))return"mi";
        if("iowa".equals(x))return"ia";
        if("north dakota".equals(x))return"nd";
        if("south dakota".equals(x))return"sd";
        return x.length()==2?x:"";
    }
    ...
    private boolean locationMatches(String value){
        ...
        for(String proof:postal.split("\\|")){
            ...
            if(proof.matches(".*\\d{5}.*")){
                String zip=proof.replaceAll(".*?(\\d{5}).*","$1");
                if(page.contains(zip))return true;
            }
            String[] p=proof.split(",");
            if(p.length>=2){
                String city=norm(p[0]);
                String state=norm(p[1]);
                String abbr=stateAbbrev(p[1]);
                if(!city.isBlank()&&pageNorm.contains(city)&&((!state.isBlank()&&pageNorm.contains(state))||(!abbr.isBlank()&&pageNorm.contains(norm(abbr)))))return true;
            }else{
                String simple=norm(proof);
                if(simple.length()>=4&&pageNorm.contains(simple))return true;
            }
        }
        return false;
    }
    ...
    private void loadTerm(int index){
        if(index<0||index>=terms.size()||authBlocked){
            finishPass();
            return;
        }
        termIndex=index;
        ...
        handler.postDelayed(()->{if(g==generation)capturePage();},2500);
    }
    ...
    private void scheduleCapture(int g){
        ...
        handler.postDelayed(()->{
            if(g==generation&&collector!=null){
                try{collector.evaluateJavascript("window.scrollBy(0,Math.max(900,window.innerHeight*.85));",null);}catch(Exception ignored){}
                capturePage();
            }
        },3200);
        handler.postDelayed(()->{
            if(g==generation&&collector!=null){
                try{collector.evaluateJavascript("window.scrollBy(0,Math.max(1100,window.innerHeight));",null);}catch(Exception ignored){}
                capturePage();
            }
        },5200);
    ...
    private void nextTerm(int g){
        if(g!=generation)return;
        int n=termIndex+1;
        if(n>=terms.size())finishPass();
        else loadTerm(n);
    }
    ...
    private void finishPass(){
        ...
        scheduledGeneration=-1;
        String status;
        if(authBlocked)status="AUTH_REQUIRED";
        else if(totalAccepted>0)status="COMPLETE_WITH_ROWS";
        else if(totalCaptured>0)status="COMPLETE_LOCATION_UNPROVEN";
        else status="COMPLETE_EMPTY";
        try{
            activity.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().putString(LAST_STATUS,status).apply();
        }catch(Exception ignored){}
        stopCollector(false);
        scoutWebView.postDelayed(
            ()->scoutWebView.evaluateJavascript("window.H38ScoutReturned&&window.H38ScoutReturned();",null),
            120
        );
    }
    ...
    private void stopCollector(boolean clearCallbacks){
        if(clearCallbacks)handler.removeCallbacksAndMessages(null);
        WebView w=collector;
        collector=null;
        if(w!=null){
            try{root.removeView(w);}catch(Exception ignored){}
            try{
                w.stopLoading();
                w.removeJavascriptInterface("AndroidH38FacebookBrowser");
                w.removeAllViews();
                w.destroy();
            }catch(Exception ignored){}
        }
...
(function(){
 try{
  ...
  function cleanUrl(v){var x=String(v||'');var m=x.match(/https?:\\/\\/[^\\s]+?\\/marketplace\\/item\\/\\d+/i);return (m?m[0]:x.split('?')[0]).replace(/\\/$/,'')}
  ...
  function goodImage(v){var x=String(v||'');return /^https:\\/\\//i.test(x)&&!/\\b(?:logo|favicon|sprite|pixel|tracking|placeholder|blank)\\b/i.test(x)}
  function pageLocation(){
    var body=String((document.body&&document.body.innerText)||''),lines=body.split('\\n');
    for(var j=0;j<Math.min(lines.length,220);j++){
      var line=String(lines[j]||'').trim(),low=line.toLowerCase(),near=low.lastIndexOf(' near ');
      if(low.indexOf('search results for')>=0&&near>=0){
        var x=line.substring(near+6).trim();
        if(x.length>2&&x.length<100)return x;
      }
    }
    var nodes=[].slice.call(document.querySelectorAll('[aria-label*="location" i],[data-testid*="location" i],button,[role="button"]'));
    for(var i=0;i<nodes.length;i++){
      var aria=String(nodes[i].getAttribute&&nodes[i].getAttribute('aria-label')||''),x=T(nodes[i]);
      if(x.length>2&&x.length<100&&x.indexOf('$')<0&&/location|radius|within/i.test(aria))return x;
    }
    return'';
  }
  function cardFor(a){
    var best=a,n=a;
    for(var up=0;up<8&&n;up++,n=n.parentElement){
      var raw=T(n);
      if(raw.length>=8&&raw.length<=900)best=n;
      if(n.querySelector&&n.querySelector('img')&&raw.length>=16&&raw.length<=650)return n;
    }
    return best;
  }
  function titleFor(a,card){
    var aria=String(a.getAttribute&&a.getAttribute('aria-label')||'').trim();
    if(aria.length>=3&&aria.length<=180&&!/^\\$/.test(aria))return aria;
    var imgs=[].slice.call(card.querySelectorAll?card.querySelectorAll('img'):[]);
    for(var i=0;i<imgs.length;i++){
      var alt=String(imgs[i].getAttribute('alt')||'').replace(/\\s+/g,' ').trim();
      if(alt.length>=3&&alt.length<=180&&!/^(image|photo|facebook|marketplace)$/i.test(alt))return alt;
    }
    var nodes=[].slice.call(card.querySelectorAll?card.querySelectorAll('[dir="auto"],span,div'):[]);
    for(var j=0;j<nodes.length;j++){
      var x=T(nodes[j]);
      if(x.length<3||x.length>180||x.charAt(0)==='$'||/\\b(?:mi|miles)\\b/i.test(x)||/^(sponsored|new listing)$/i.test(x))continue;
      return x;
    }
  ...
  function imageFor(card){
    var imgs=[].slice.call(card.querySelectorAll?card.querySelectorAll('img'):[]);
    for(var i=0;i<imgs.length;i++){
      var u=String(imgs[i].currentSrc||imgs[i].src||'');
      if(goodImage(u))return u;
    }
    return'';
  }
  ...
  anchors.forEach(function(a){
    var href=String(a.href||a.getAttribute('href')||''),id=itemId(href);
    if(!id||seen[id])return;
    seen[id]=1;
    var card=cardFor(a),raw=T(card||a);
    if(!raw)return;
    rows.push({
      id:id,
      source:'Facebook Marketplace',
      title:titleFor(a,card||a),
      text:raw,
      price:price(raw),
      url:cleanUrl(href),
      image_url:imageFor(card||a),
      distance_miles:distance(raw),
      captured_at:Date.now(),
      browser_session:true
    });
  });
  AndroidH38FacebookBrowser.capture(JSON.stringify({
    login_required:false,
    location_text:pageLocation(),
    rows:rows,
    item_urls_seen:rows.length
  }));
 }catch(e){
  AndroidH38FacebookBrowser.capture(JSON.stringify({error:String(e),rows:[]}));
 }
})();
""";
        ...
        @JavascriptInterface public void capture(String json){
            activity.runOnUiThread(()->{
                try{
                    ...
                    JSONArray incoming=p.optJSONArray("rows");
                    if(incoming==null)return;
                    String pageLoc=p.optString("location_text","");
                    boolean pageProven=locationMatches(pageLoc);
                    totalCaptured+=incoming.length();
                    JSONArray preserved=new JSONArray();

                    for(int i=0;i<incoming.length();i++){
                        JSONObject x=incoming.optJSONObject(i);
                        if(x==null)continue;
                        double d=x.has("distance_miles")&&!x.isNull("distance_miles")
                            ?x.optDouble("distance_miles",Double.NaN):Double.NaN;
                        boolean hasDistance=Double.isFinite(d);
                        boolean distanceProven=hasDistance&&d<=radiusMiles;
                        boolean outsideRadius=hasDistance&&d>radiusMiles;
                        boolean cardProven=locationMatches(x.optString("text",""));
                        boolean verified=!outsideRadius&&(distanceProven||cardProven||pageProven);

                        x.put("distance_verified",distanceProven);
                        x.put("page_location_verified",pageProven);
                        x.put("card_location_verified",cardProven);
                        x.put("location_verified",verified);
                        x.put("facebook_marketplace_location",pageLoc);
                        if(outsideRadius)x.put("location_status","OUTSIDE_RADIUS");
                        else if(distanceProven)x.put("location_status","DISTANCE_VERIFIED");
                        else if(cardProven)x.put("location_status","CARD_LOCATION_VERIFIED");
                        else if(pageProven)x.put("location_status","PAGE_LOCATION_VERIFIED");
                        else x.put("location_status","LOCATION_UNPROVEN");
                        if(verified)totalAccepted++;
                        preserved.put(x);
                    }
    ...
    private static int mergeRows(Context context,JSONArray incoming,String term,String postal,int radius,String locationText){
        ...
        try{
            ...
            for(int i=0;i<old.length();i++){
                ...
                if(x==null)continue;
                String id=x.optString("id",x.optString("url",""));
            ...
            for(int i=0;i<incoming.length();i++){
                ...
                if(x==null)continue;
                String id=x.optString("id",x.optString("url",""));
                if(id.isBlank()||!incomingIds.add(id))continue;
                x.put("term",term==null?"":term);
                x.put("captured_at",System.currentTimeMillis());
                x.put("search_postal",postal==null?"":postal);
                x.put("search_radius_miles",radius);
                if(!x.has("facebook_marketplace_location"))x.put("facebook_marketplace_location",locationText==null?"":locationText);
            ...
            int start=Math.max(0,rows.size()-MAX_ROWS);
            for(int i=start;i<rows.size();i++)save.put(rows.get(i));
    ...
    static String rowsJson(Context context){
        try{
            SharedPreferences p=context.getSharedPreferences(PREFS,Context.MODE_PRIVATE);
            String raw=p.getString(ROWS,"[]");
            String postal=p.getString(LAST_POSTAL,"");
            String status=p.getString(LAST_STATUS,"");
            JSONArray old=new JSONArray(raw);
            JSONArray safe=new JSONArray();
            int captured=0,usable=0,unproven=0,outside=0;

            for(int i=0;i<old.length();i++){
                JSONObject x=old.optJSONObject(i);
                if(x==null)continue;
                if(!postal.isBlank()&&!postal.equals(x.optString("search_postal","")))continue;
                captured++;
                if(x.optBoolean("location_verified",false))usable++;
                else if("OUTSIDE_RADIUS".equals(x.optString("location_status","")))outside++;
                else unproven++;
            }

            if(!status.isBlank()){
                JSONObject marker=new JSONObject();
                marker.put("h38_system",true);
                marker.put("status",status);
                marker.put("location_verified",false);
                marker.put("captured_count",captured);
                marker.put("usable_count",usable);
                marker.put("location_unproven_count",unproven);
                marker.put("outside_radius_count",outside);
                safe.put(marker);
            }
