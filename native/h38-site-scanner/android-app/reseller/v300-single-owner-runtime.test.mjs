...
const v314=fs.readFileSync(path.join(p,'v314-source-quality-repair.js'),'utf8');
...
assert.match(v265,/H38_SCOUT_V313_SOURCE_POLISH=true/);
...
assert.match(v265,/genericDgTitle/);
assert.match(v265,/Product name not resolved · UPC/);
assert.match(v265,/baseFacebookSnapshotV313/);
assert.match(v265,/function listingText\(r\)\{return normalize\(`\$\{r\?\.title\|\|''\} \$\{r\?\.text\|\|''\} \$\{r\?\.description\|\|''\} \$\{r\?\.category\|\|''\} \$\{r\?\.subcategory\|\|''\}`\)\}/);
assert.doesNotMatch(v265,/listingText\(r\).*r\?\.term/);
assert.match(v265,/relevant \/ \$\{captured\.length\} captured/);
assert.match(v265,/query-tagged but unrelated public card/);
...
assert.match(v314,/H38_SCOUT_V315_FIREBASE_STABILITY=true/);
assert.match(v314,/queryPinned/);
assert.match(v314,/No sale leads loaded yet\. No ordinary classifieds were relabeled as sales\./);
assert.match(v314,/saveQuery\(input\?input\.value:state\.discover\?\.query\|\|''\)/);
...
assert.match(main,/handleMainRendererGone/);
assert.match(main,/backend_preferred_v315/);
assert.match(main,/bypassAutomaticDollarGeneralCheck/);
...
assert.match(gradle,/versionName '3\.0\.15'/);
assert.match(rules,/Single-owner runtime rule/);
assert.match(rules,/Physical Android phone behavior remains final acceptance/);
console.log('PASS v3.0.15 Firebase stability + source ownership + query preservation + renderer containment contracts');
