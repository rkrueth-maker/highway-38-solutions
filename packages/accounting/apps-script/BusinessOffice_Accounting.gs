/** Business Office Platform — double-entry preparation, posting, periods, corrections, voids, and reports. */
...
function boPrepareJournalEntry(payload) {
  return boSafeExecute_('Prepare journal entry', function () {
    ...
    boRequirePermission_(BO_SHEETS.JOURNAL_ENTRIES, 'Create');
    ...
    const entry = boAppendRecord_(BO_SHEETS.JOURNAL_ENTRIES, {
      'Journal Entry ID': entryId,
      'Entry Number': entryNumber,
      'Entry Date': payload.entryDate || Utilities.formatDate(new Date(), boGetTimeZone_(), 'yyyy-MM-dd'),
    ...
    payload.lines.forEach(function (line, index) {
      ...
      boAppendRecord_(BO_SHEETS.JOURNAL_LINES, {
...
function boFindAccount_(accountCode) {
  const account = boReadTable_(BO_SHEETS.CHART_OF_ACCOUNTS, { includeVoided: true }).find(function (row) {
...
function boGetAccountingPeriod_(periodId) {
  const period = boFindRecord_(BO_SHEETS.ACCOUNTING_PERIODS, periodId, { includeVoided: true }).record;
...
function boPostJournalEntry(entryId) {
  return boSafeExecute_('Post journal entry', function () {
    const user = boRequireRestrictedArea_('posting');
    boRequirePermission_(BO_SHEETS.JOURNAL_ENTRIES, 'Post');
    const entry = boFindRecord_(BO_SHEETS.JOURNAL_ENTRIES, entryId, { includeVoided: true }).record;
    ...
    const updated = boUpdateRecord_(BO_SHEETS.JOURNAL_ENTRIES, entryId, {
      Status: 'Posted',
...
function boPrepareInvoicePosting(invoiceId) {
  const invoice = boFindRecord_(BO_SHEETS.INVOICES, invoiceId).record;
...
function boPrepareExpensePosting(expenseId) {
  const expense = boFindRecord_(BO_SHEETS.EXPENSES, expenseId).record;
...
function boPreparePaymentPosting(paymentId) {
  const payment = boFindRecord_(BO_SHEETS.PAYMENTS, paymentId).record;
...
function boResolveAccountingPeriod_(dateValue) {
  ...
  const period = boReadTable_(BO_SHEETS.ACCOUNTING_PERIODS, { includeVoided: true }).find(function (row) {
...
function boReverseJournalEntry(entryId, reason) {
  return boSafeExecute_('Reverse journal entry', function () {
    ...
    const lines = boReadTable_(BO_SHEETS.JOURNAL_LINES, { includeVoided: true }).filter(function (line) {
    ...
    const reversal = boPrepareJournalEntry({
      entryDate: Utilities.formatDate(new Date(), boGetTimeZone_(), 'yyyy-MM-dd'),
      sourceType: 'Reversal',
      sourceId: entryId,
      description: 'Reverse ' + entry['Entry Number'] + ': ' + boNormalizeText_(reason),
      accountingPeriodId: boResolveAccountingPeriod_(Utilities.formatDate(new Date(), boGetTimeZone_(), 'yyyy-MM-dd')),
      lines: lines
    });
    boUpdateRecord_(BO_SHEETS.JOURNAL_ENTRIES, entryId, { 'Reversal Entry ID': reversal['Journal Entry ID'] }, 'Journal reversal');
...
function boLockAccountingPeriod(periodId, notes) {
  ...
  const unposted = boReadTable_(BO_SHEETS.JOURNAL_ENTRIES, { includeVoided: true }).filter(function (entry) {
  ...
  const updated = boUpdateRecord_(BO_SHEETS.ACCOUNTING_PERIODS, periodId, {
    Status: 'Locked',
...
function boGetAccountingReports() {
  ...
  return {
    profitAndLoss: boReadTable_(BO_SHEETS.PNL, { allBusinesses: true, includeVoided: true }),
    balanceSheet: boReadTable_(BO_SHEETS.BALANCE_SHEET, { allBusinesses: true, includeVoided: true }),
    cashFlow: boReadTable_(BO_SHEETS.CASH_FLOW, { allBusinesses: true, includeVoided: true }),
    arAging: boReadTable_(BO_SHEETS.AR_AGING, { allBusinesses: true, includeVoided: true }),
    apAging: boReadTable_(BO_SHEETS.AP_AGING, { allBusinesses: true, includeVoided: true }),
    jobProfitability: boReadTable_(BO_SHEETS.JOB_PROFITABILITY, { allBusinesses: true, includeVoided: true })
  };
}

function boValidateLedger() {
  boRequirePermission_('Accounting Reports', 'View');
  const entries = boReadTable_(BO_SHEETS.JOURNAL_ENTRIES, { includeVoided: true });
  ...
  const pnl = boGetSheet_(BO_SHEETS.PNL).getRange('A1:B8').getDisplayValues();
  const balanceSheet = boGetSheet_(BO_SHEETS.BALANCE_SHEET).getRange('A1:B9').getDisplayValues();
